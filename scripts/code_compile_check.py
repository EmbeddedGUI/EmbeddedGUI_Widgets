"""Compile-check helper for the standalone EmbeddedGUI custom widgets repository."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Compile standalone HelloCustomWidgets demos and the custom-widget unit-test harness.",
    ...
    parser.add_argument("--full-check",
                        action="store_true",
                        default=False,
                        help="Compile all HelloCustomWidgets demos, then build and run the custom-widget unit-test harness.")
...
if __name__ == '__main__':
    ...
    if args.full_check:
        res = run_touch_release_semantics_check(scope='custom')
        if res != 0:
            sys.exit(res)
