...
#include "test_group_box.h"

#include "../../HelloCustomWidgets/layout/group_box/egui_view_group_box.h"
#include "../../HelloCustomWidgets/layout/group_box/egui_view_group_box.c"

typedef struct group_box_preview_snapshot group_box_preview_snapshot_t;
struct group_box_preview_snapshot
{
...
static egui_view_group_box_t preview_control;
...
static void setup_group_box(void)
{
    egui_view_group_box_init(EGUI_VIEW_OF(&test_control));
    egui_view_set_size(EGUI_VIEW_OF(&test_control), 150, 100);
}

static void setup_preview_control(void)
{
    egui_view_group_box_init(EGUI_VIEW_OF(&preview_control));
    egui_view_set_size(EGUI_VIEW_OF(&preview_control), 92, 54);
    setup_label(&preview_header, 62, 12, "Compact");
    setup_label(&preview_content, 68, 16, "Top left");
    egui_view_group_box_set_header(EGUI_VIEW_OF(&preview_control), EGUI_VIEW_OF(&preview_header));
    egui_view_group_box_set_content(EGUI_VIEW_OF(&preview_control), EGUI_VIEW_OF(&preview_content));
    egui_view_group_box_apply_compact_style(EGUI_VIEW_OF(&preview_control));
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&preview_control), on_preview_click);
    egui_view_group_box_override_static_preview_api(EGUI_VIEW_OF(&preview_control), &preview_api);
    click_count = 0;
}
...
static void capture_preview_snapshot(group_box_preview_snapshot_t *snapshot)
{
    ...
    snapshot->header_surface_color = preview_control.header_surface_color;
    snapshot->content_surface_color = preview_control.content_surface_color;
    ...
    snapshot->header_indent = preview_control.header_indent;
...
static void assert_preview_state_unchanged(const group_box_preview_snapshot_t *snapshot)
{
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->header_surface_color.full, preview_control.header_surface_color.full);
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->content_surface_color.full, preview_control.content_surface_color.full);
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->header_indent, preview_control.header_indent);
...
static void test_group_box_init_defaults(void)
{
    setup_group_box();
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(7, egui_view_group_box_get_header_gap(EGUI_VIEW_OF(&test_control)));
...
static void test_group_box_header_and_content_layout(void)
{
    ...
    egui_view_group_box_set_header(EGUI_VIEW_OF(&test_control), EGUI_VIEW_OF(&test_header));
    egui_view_group_box_set_content(EGUI_VIEW_OF(&test_control), EGUI_VIEW_OF(&test_content));
    ...
    EGUI_TEST_ASSERT_TRUE(egui_view_group_box_get_header(EGUI_VIEW_OF(&test_control)) == NULL);
    EGUI_TEST_ASSERT_TRUE(egui_view_group_box_get_content(EGUI_VIEW_OF(&test_control)) == EGUI_VIEW_OF(&test_header));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_group_get_child_count(EGUI_VIEW_OF(&test_control)));
}

static void test_group_box_styles_palette_and_clamps(void)
{
    setup_group_box();

    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_group_box_set_corner_radius(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(18, egui_view_group_box_get_corner_radius(EGUI_VIEW_OF(&test_control)));

    egui_view_group_box_set_border_width(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(3, egui_view_group_box_get_border_width(EGUI_VIEW_OF(&test_control)));

    egui_view_group_box_set_header_gap(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(16, egui_view_group_box_get_header_gap(EGUI_VIEW_OF(&test_control)));

    egui_view_group_box_set_header_indent(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(28, egui_view_group_box_get_header_indent(EGUI_VIEW_OF(&test_control)));

    egui_view_group_box_set_palette(EGUI_VIEW_OF(&test_control), EGUI_COLOR_HEX(0x010203), EGUI_COLOR_HEX(0x111213),
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x212223).full, test_control.header_surface_color.full);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x313233).full, test_control.content_surface_color.full);
...
{
    group_box_preview_snapshot_t initial_snapshot;
    egui_dim_t center_x;
    egui_dim_t center_y;
...
void test_group_box_run(void)
{
    EGUI_TEST_SUITE_BEGIN(group_box);
    EGUI_TEST_RUN(test_group_box_init_defaults);
    EGUI_TEST_RUN(test_group_box_header_and_content_layout);
    EGUI_TEST_RUN(test_group_box_styles_palette_and_clamps);
    EGUI_TEST_RUN(test_group_box_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_SUITE_END();
}
