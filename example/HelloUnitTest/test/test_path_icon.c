...
static void setup_preview_path_icon(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&preview_path_icon), 24, 24);
...
static void test_path_icon_static_preview_consumes_input(void)
{
    egui_dim_t center_x;
    egui_dim_t center_y;

    setup_preview_path_icon();
    layout_view(EGUI_VIEW_OF(&preview_path_icon), 12, 18, 24, 24);
    get_view_center(EGUI_VIEW_OF(&preview_path_icon), &center_x, &center_y);

    egui_view_set_pressed(EGUI_VIEW_OF(&preview_path_icon), 1);
    EGUI_TEST_ASSERT_TRUE(send_touch(EGUI_VIEW_OF(&preview_path_icon), EGUI_MOTION_EVENT_ACTION_DOWN, center_x, center_y));
    EGUI_TEST_ASSERT_TRUE(send_touch(EGUI_VIEW_OF(&preview_path_icon), EGUI_MOTION_EVENT_ACTION_UP, center_x, center_y));
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&preview_path_icon)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(0, g_click_count);

    egui_view_set_pressed(EGUI_VIEW_OF(&preview_path_icon), 1);
    EGUI_TEST_ASSERT_TRUE(send_key(EGUI_VIEW_OF(&preview_path_icon), EGUI_KEY_EVENT_ACTION_DOWN, EGUI_KEY_CODE_ENTER));
    EGUI_TEST_ASSERT_TRUE(send_key(EGUI_VIEW_OF(&preview_path_icon), EGUI_KEY_EVENT_ACTION_UP, EGUI_KEY_CODE_ENTER));
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&preview_path_icon)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(0, g_click_count);
}
...
void test_path_icon_run(void)
{
    ...
    EGUI_TEST_RUN(test_path_icon_static_preview_consumes_input);
    EGUI_TEST_SUITE_END();
}
