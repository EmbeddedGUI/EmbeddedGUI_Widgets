...
#include "test_content_control.h"

#include "../../HelloCustomWidgets/layout/content_control/egui_view_content_control.h"
#include "../../HelloCustomWidgets/layout/content_control/egui_view_content_control.c"

typedef struct content_control_preview_snapshot content_control_preview_snapshot_t;
struct content_control_preview_snapshot
{
...
static egui_view_content_control_t test_control;
static egui_view_label_t test_child;
static egui_view_content_control_t preview_control;
...
static void setup_content_control(void)
{
    egui_view_content_control_init(EGUI_VIEW_OF(&test_control));
    egui_view_set_size(EGUI_VIEW_OF(&test_control), 140, 80);
}

static void setup_preview_control(void)
{
    egui_view_content_control_init(EGUI_VIEW_OF(&preview_control));
    egui_view_set_size(EGUI_VIEW_OF(&preview_control), 88, 44);
    setup_child(&preview_child, 64, 18, "Compact");
    egui_view_content_control_set_child(EGUI_VIEW_OF(&preview_control), EGUI_VIEW_OF(&preview_child));
    egui_view_content_control_apply_compact_style(EGUI_VIEW_OF(&preview_control));
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&preview_control), on_preview_click);
    egui_view_content_control_override_static_preview_api(EGUI_VIEW_OF(&preview_control), &preview_api);
    click_count = 0;
}
...
static void capture_preview_snapshot(content_control_preview_snapshot_t *snapshot)
{
...
static void assert_preview_state_unchanged(const content_control_preview_snapshot_t *snapshot)
{
...
static void test_content_control_init_defaults(void)
{
    setup_content_control();

    EGUI_TEST_ASSERT_TRUE(egui_view_content_control_get_child(EGUI_VIEW_OF(&test_control)) == NULL);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_ALIGN_CENTER, egui_view_content_control_get_content_align_type(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(10, egui_view_content_control_get_corner_radius(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_content_control_get_border_width(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_content_control_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_content_control_get_read_only_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0xFFFFFF).full, test_control.surface_color.full);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0xC9D4DE).full, test_control.border_color.full);
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(16, EGUI_VIEW_OF(&test_control)->padding.left);
    EGUI_TEST_ASSERT_EQUAL_INT(16, EGUI_VIEW_OF(&test_control)->padding.right);
    EGUI_TEST_ASSERT_EQUAL_INT(12, EGUI_VIEW_OF(&test_control)->padding.top);
    EGUI_TEST_ASSERT_EQUAL_INT(12, EGUI_VIEW_OF(&test_control)->padding.bottom);
#endif
}

static void test_content_control_child_alignment_layout(void)
{
    setup_content_control();
    setup_child(&test_child, 48, 20, "Child");

    egui_view_content_control_set_child(EGUI_VIEW_OF(&test_control), EGUI_VIEW_OF(&test_child));
    EGUI_TEST_ASSERT_TRUE(egui_view_content_control_get_child(EGUI_VIEW_OF(&test_control)) == EGUI_VIEW_OF(&test_child));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_group_get_child_count(EGUI_VIEW_OF(&test_control)));
#if EGUI_CONFIG_FUNCTION_SUPPORT_MARGIN_PADDING
    EGUI_TEST_ASSERT_EQUAL_INT(30, EGUI_VIEW_OF(&test_child)->region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(18, EGUI_VIEW_OF(&test_child)->region.location.y);
#else
    ...
    egui_view_content_control_set_content_align_type(EGUI_VIEW_OF(&test_control), EGUI_ALIGN_BOTTOM_RIGHT);
#if EGUI_CONFIG_FUNCTION_SUPPORT_MARGIN_PADDING
    EGUI_TEST_ASSERT_EQUAL_INT(60, EGUI_VIEW_OF(&test_child)->region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(36, EGUI_VIEW_OF(&test_child)->region.location.y);
#else
...
static void test_content_control_styles_palette_and_clamps(void)
{
    setup_content_control();

    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_content_control_set_corner_radius(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(22, egui_view_content_control_get_corner_radius(EGUI_VIEW_OF(&test_control)));

    egui_view_content_control_set_border_width(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(3, egui_view_content_control_get_border_width(EGUI_VIEW_OF(&test_control)));

    egui_view_content_control_set_palette(EGUI_VIEW_OF(&test_control), EGUI_COLOR_HEX(0x010203), EGUI_COLOR_HEX(0x111213),
    ...
    egui_view_content_control_apply_leading_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, egui_view_content_control_get_content_align_type(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_content_control_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_content_control_get_read_only_mode(EGUI_VIEW_OF(&test_control)));

    egui_view_content_control_apply_compact_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_content_control_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_content_control_get_read_only_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_ALIGN_TOP_LEFT, egui_view_content_control_get_content_align_type(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(6, egui_view_content_control_get_corner_radius(EGUI_VIEW_OF(&test_control)));

    egui_view_content_control_apply_read_only_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_content_control_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_content_control_get_read_only_mode(EGUI_VIEW_OF(&test_control)));
}

static void test_content_control_static_preview_consumes_input_and_keeps_state(void)
{
    content_control_preview_snapshot_t initial_snapshot;
    egui_dim_t center_x;
    egui_dim_t center_y;

    setup_preview_control();
    layout_view(EGUI_VIEW_OF(&preview_control), 12, 18, 88, 44);
...
void test_content_control_run(void)
{
    EGUI_TEST_SUITE_BEGIN(content_control);
    EGUI_TEST_RUN(test_content_control_init_defaults);
    EGUI_TEST_RUN(test_content_control_child_alignment_layout);
    EGUI_TEST_RUN(test_content_control_styles_palette_and_clamps);
    EGUI_TEST_RUN(test_content_control_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_SUITE_END();
}
