...
#include "../../HelloCustomWidgets/display/line/egui_view_shape_line.h"
#include "../../HelloCustomWidgets/display/line/egui_view_shape_line.c"

typedef struct line_preview_snapshot line_preview_snapshot_t;
struct line_preview_snapshot
{
...
static egui_view_shape_line_t test_control;
static egui_view_shape_line_t preview_control;
...
{
    egui_view_shape_line_init(EGUI_VIEW_OF(&test_control));
...
static void setup_preview_control(void)
{
    ...
    egui_view_shape_line_set_palette(EGUI_VIEW_OF(&preview_control), HCW_COLOR_PRIMARY, HCW_COLOR_PRIMARY_TINT);
...
static void capture_preview_snapshot(line_preview_snapshot_t *snapshot)
{
    ...
    snapshot->x1_percent = preview_control.x1_percent;
    snapshot->y1_percent = preview_control.y1_percent;
...
static void assert_preview_state_unchanged(const line_preview_snapshot_t *snapshot)
{
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->x1_percent, preview_control.x1_percent);
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->y1_percent, preview_control.y1_percent);
...
static void assert_points(egui_view_t *view, uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
    uint8_t actual_x1 = 0;
    uint8_t actual_y1 = 0;
    uint8_t actual_x2 = 0;
    ...
    egui_view_shape_line_get_points(view, &actual_x1, &actual_y1, &actual_x2, &actual_y2);
    EGUI_TEST_ASSERT_EQUAL_INT(x1, actual_x1);
    EGUI_TEST_ASSERT_EQUAL_INT(y1, actual_y1);
    EGUI_TEST_ASSERT_EQUAL_INT(x2, actual_x2);
    EGUI_TEST_ASSERT_EQUAL_INT(y2, actual_y2);
}

static void test_line_init_defaults(void)
{
    setup_line();
...
static void test_line_setters_clamp_and_clear_pressed_state(void)
{
    setup_line();
    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_shape_line_set_stroke_width(EGUI_VIEW_OF(&test_control), -3);
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_shape_line_get_stroke_width(EGUI_VIEW_OF(&test_control)));

    egui_view_shape_line_set_stroke_width(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(6, egui_view_shape_line_get_stroke_width(EGUI_VIEW_OF(&test_control)));

    egui_view_shape_line_set_points(EGUI_VIEW_OF(&test_control), 7, 8, 120, 130);
    assert_points(EGUI_VIEW_OF(&test_control), 7, 8, 100, 100);

    egui_view_shape_line_set_palette(EGUI_VIEW_OF(&test_control), EGUI_COLOR_HEX(0x111213), EGUI_COLOR_HEX(0x212223));
...
static void test_line_styles(void)
{
    setup_line();

    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_shape_line_apply_accent_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    ...
    egui_view_shape_line_set_palette(EGUI_VIEW_OF(&test_control), HCW_COLOR_PRIMARY, HCW_COLOR_PRIMARY_TINT);
    ...
    egui_view_shape_line_set_palette(EGUI_VIEW_OF(&test_control), HCW_COLOR_TEXT_SOFT, HCW_COLOR_BORDER_STRONG);
    egui_view_shape_line_set_stroke_width(EGUI_VIEW_OF(&test_control), 1);
    egui_view_shape_line_set_points(EGUI_VIEW_OF(&test_control), 12, 50, 88, 50);
    assert_points(EGUI_VIEW_OF(&test_control), 12, 50, 88, 50);
}

static void test_line_static_preview_consumes_input_and_keeps_state(void)
{
    line_preview_snapshot_t initial_snapshot;
    egui_dim_t center_x;
    egui_dim_t center_y;
...
void test_line_run(void)
{
    EGUI_TEST_SUITE_BEGIN(line);
    EGUI_TEST_RUN(test_line_init_defaults);
    EGUI_TEST_RUN(test_line_setters_clamp_and_clear_pressed_state);
    EGUI_TEST_RUN(test_line_styles);
    EGUI_TEST_RUN(test_line_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_SUITE_END();
}
