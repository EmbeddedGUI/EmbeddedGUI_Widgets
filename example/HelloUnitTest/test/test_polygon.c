...
#include "../../HelloCustomWidgets/display/polygon/egui_view_polygon.h"
#include "../../HelloCustomWidgets/display/polygon/egui_view_polygon.c"

typedef struct polygon_preview_snapshot polygon_preview_snapshot_t;
struct polygon_preview_snapshot
{
...
static void setup_polygon(void)
{
    egui_view_polygon_init(EGUI_VIEW_OF(&test_control));
    egui_view_set_size(EGUI_VIEW_OF(&test_control), 118, 70);
}
...
static void capture_preview_snapshot(polygon_preview_snapshot_t *snapshot)
{
...
static void assert_preview_state_unchanged(const polygon_preview_snapshot_t *snapshot)
{
...
static void assert_point(egui_view_t *view, uint8_t index, uint8_t x, uint8_t y)
{
    ...
    egui_view_polygon_get_point(view, index, &actual_x, &actual_y);
    EGUI_TEST_ASSERT_EQUAL_INT(x, actual_x);
    EGUI_TEST_ASSERT_EQUAL_INT(y, actual_y);
}

static void test_polygon_init_defaults(void)
{
    setup_polygon();

    EGUI_TEST_ASSERT_EQUAL_INT(2, egui_view_polygon_get_stroke_width(EGUI_VIEW_OF(&test_control)));
...
static void test_polygon_setters_clamp_and_clear_pressed_state(void)
{
    ...
    egui_view_polygon_set_stroke_width(EGUI_VIEW_OF(&test_control), -3);
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_polygon_get_stroke_width(EGUI_VIEW_OF(&test_control)));

    egui_view_polygon_set_stroke_width(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(6, egui_view_polygon_get_stroke_width(EGUI_VIEW_OF(&test_control)));

    egui_view_polygon_set_points(EGUI_VIEW_OF(&test_control), points, 9);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_POLYGON_MAX_POINTS, egui_view_polygon_get_point_count(EGUI_VIEW_OF(&test_control)));
    ...
    egui_view_polygon_set_palette(EGUI_VIEW_OF(&test_control), EGUI_COLOR_HEX(0x111213), EGUI_COLOR_HEX(0x212223), EGUI_COLOR_HEX(0x313233));
    ...
    egui_view_polygon_set_points(EGUI_VIEW_OF(&test_control), NULL, 4);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_polygon_get_point_count(EGUI_VIEW_OF(&test_control)));
    assert_point(EGUI_VIEW_OF(&test_control), 1, 0, 0);
}

static void test_polygon_styles(void)
{
    setup_polygon();

    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_polygon_apply_accent_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(3, egui_view_polygon_get_stroke_width(EGUI_VIEW_OF(&test_control)));
...
static void test_polygon_static_preview_consumes_input_and_keeps_state(void)
{
    polygon_preview_snapshot_t initial_snapshot;
    egui_dim_t center_x;
    egui_dim_t center_y;

    setup_preview_control();
    layout_view(EGUI_VIEW_OF(&preview_control), 12, 18, 68, 38);
...
void test_polygon_run(void)
{
    EGUI_TEST_SUITE_BEGIN(polygon);
    EGUI_TEST_RUN(test_polygon_init_defaults);
    EGUI_TEST_RUN(test_polygon_setters_clamp_and_clear_pressed_state);
    EGUI_TEST_RUN(test_polygon_styles);
    EGUI_TEST_RUN(test_polygon_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_SUITE_END();
}
