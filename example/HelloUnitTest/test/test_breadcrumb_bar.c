...
static void test_breadcrumb_bar_set_snapshots_clamp_and_reset_current(void)
{
    ...
    EGUI_VIEW_OF(&test_bar)->is_pressed = true;
...
static void test_breadcrumb_bar_read_only_mode_clears_pressed_and_ignores_input(void)
{
    setup_bar();
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(0, test_bar.read_only_mode);
    EGUI_TEST_ASSERT_TRUE(send_touch(EGUI_MOTION_EVENT_ACTION_DOWN));
    EGUI_TEST_ASSERT_TRUE(send_touch(EGUI_MOTION_EVENT_ACTION_UP));
    EGUI_TEST_ASSERT_EQUAL_INT(1, click_count);
}

static void test_breadcrumb_bar_disabled_ignores_input_and_clears_pressed_state(void)
{
    setup_bar();
...
void test_breadcrumb_bar_run(void)
{
    ...
    EGUI_TEST_RUN(test_breadcrumb_bar_read_only_mode_clears_pressed_and_ignores_input);
    EGUI_TEST_RUN(test_breadcrumb_bar_disabled_ignores_input_and_clears_pressed_state);
