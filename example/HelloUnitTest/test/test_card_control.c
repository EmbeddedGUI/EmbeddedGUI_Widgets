...
#include "test_card_control.h"

#include "../../HelloCustomWidgets/layout/card_control/egui_view_card_control.h"
#include "../../HelloCustomWidgets/layout/card_control/egui_view_card_control.c"

static egui_view_card_control_t test_widget;
static egui_view_card_control_t preview_widget;
static egui_view_api_t preview_api;
static uint8_t g_action_count;
static uint8_t g_action_snapshot;
static uint8_t g_action_part;

static const egui_view_card_control_snapshot_t g_snapshots[] = {
...
static const egui_view_card_control_snapshot_t g_overflow_snapshots[] = {
        {"A", "A", "A", "A", "A", "A", EGUI_VIEW_CARD_CONTROL_TONE_ACCENT, 1, EGUI_VIEW_CARD_CONTROL_CONTROL_VALUE},
        {"B", "B", "B", "B", NULL, "B", EGUI_VIEW_CARD_CONTROL_TONE_SUCCESS, 0, EGUI_VIEW_CARD_CONTROL_CONTROL_SWITCH_ON},
        {"C", "C", "C", "C", NULL, "C", EGUI_VIEW_CARD_CONTROL_TONE_WARNING, 1, EGUI_VIEW_CARD_CONTROL_CONTROL_CHEVRON},
        {"D", "D", "D", "D", "D", "D", EGUI_VIEW_CARD_CONTROL_TONE_NEUTRAL, 0, EGUI_VIEW_CARD_CONTROL_CONTROL_VALUE},
        {"E", "E", "E", "E", NULL, "E", EGUI_VIEW_CARD_CONTROL_TONE_ACCENT, 1, EGUI_VIEW_CARD_CONTROL_CONTROL_SWITCH_OFF},
        {"F", "F", "F", "F", "F", "F", EGUI_VIEW_CARD_CONTROL_TONE_SUCCESS, 0, EGUI_VIEW_CARD_CONTROL_CONTROL_VALUE},
        {"G", "G", "G", "G", NULL, "G", EGUI_VIEW_CARD_CONTROL_TONE_WARNING, 1, EGUI_VIEW_CARD_CONTROL_CONTROL_NONE},
};
...
static void reset_action_state(void)
{
    ...
    g_action_part = EGUI_VIEW_CARD_CONTROL_PART_NONE;
}

static void setup_widget(const egui_view_card_control_snapshot_t *snapshots, uint8_t snapshot_count)
{
    egui_view_card_control_init(EGUI_VIEW_OF(&test_widget));
    egui_view_set_size(EGUI_VIEW_OF(&test_widget), 196, 102);
    egui_view_card_control_set_snapshots(EGUI_VIEW_OF(&test_widget), snapshots, snapshot_count);
    egui_view_card_control_set_font(EGUI_VIEW_OF(&test_widget), NULL);
    egui_view_card_control_set_meta_font(EGUI_VIEW_OF(&test_widget), NULL);
    egui_view_card_control_set_on_action_listener(EGUI_VIEW_OF(&test_widget), on_action);
    reset_action_state();
}

static void setup_preview_widget(void)
{
    egui_view_card_control_init(EGUI_VIEW_OF(&preview_widget));
    egui_view_set_size(EGUI_VIEW_OF(&preview_widget), 104, 74);
    egui_view_card_control_set_snapshots(EGUI_VIEW_OF(&preview_widget), &g_snapshots[0], 1);
    egui_view_card_control_set_font(EGUI_VIEW_OF(&preview_widget), NULL);
    egui_view_card_control_set_meta_font(EGUI_VIEW_OF(&preview_widget), NULL);
    egui_view_card_control_set_compact_mode(EGUI_VIEW_OF(&preview_widget), 1);
    egui_view_card_control_override_static_preview_api(EGUI_VIEW_OF(&preview_widget), &preview_api);
    reset_action_state();
}
...
{
    layout_view(EGUI_VIEW_OF(&test_widget), 10, 20, 196, 102);
}
...
static uint8_t get_part_center(egui_view_t *view, uint8_t part, egui_dim_t *x, egui_dim_t *y)
{
    ...
    if (!egui_view_card_control_get_part_region(view, part, &region))
    {
        return 0;
    }
...
static void seed_pressed_state(egui_view_card_control_t *widget, uint8_t part, uint8_t visual_pressed)
{
    widget->pressed_part = part;
    egui_view_set_pressed(EGUI_VIEW_OF(widget), visual_pressed ? 1 : 0);
}

static void assert_pressed_cleared(egui_view_card_control_t *widget)
{
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_NONE, widget->pressed_part);
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(widget)->is_pressed);
}

static void test_card_control_set_snapshots_and_current_part_clamp(void)
{
    setup_widget(g_overflow_snapshots, EGUI_ARRAY_SIZE(g_overflow_snapshots));

    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_MAX_SNAPSHOTS, test_widget.snapshot_count);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));

    seed_pressed_state(&test_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 1);
    test_widget.current_snapshot = 5;
    egui_view_card_control_set_snapshots(EGUI_VIEW_OF(&test_widget), g_snapshots, 1);
    EGUI_TEST_ASSERT_EQUAL_INT(1, test_widget.snapshot_count);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));
    assert_pressed_cleared(&test_widget);

    egui_view_card_control_set_current_part(EGUI_VIEW_OF(&test_widget), EGUI_VIEW_CARD_CONTROL_PART_NONE);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));

    egui_view_card_control_set_snapshots(EGUI_VIEW_OF(&test_widget), NULL, 0);
    EGUI_TEST_ASSERT_EQUAL_INT(0, test_widget.snapshot_count);
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_NONE, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));
}

static void test_card_control_setters_clear_pressed_and_update_state(void)
{
    ...
    seed_pressed_state(&test_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 0);
    egui_view_card_control_set_meta_font(EGUI_VIEW_OF(&test_widget), NULL);
    EGUI_TEST_ASSERT_TRUE(test_widget.meta_font == (const egui_font_t *)EGUI_CONFIG_FONT_DEFAULT);
    assert_pressed_cleared(&test_widget);

    seed_pressed_state(&test_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 1);
    egui_view_card_control_set_compact_mode(EGUI_VIEW_OF(&test_widget), 2);
    EGUI_TEST_ASSERT_EQUAL_INT(1, test_widget.compact_mode);
    assert_pressed_cleared(&test_widget);

    seed_pressed_state(&test_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 1);
    egui_view_card_control_set_read_only_mode(EGUI_VIEW_OF(&test_widget), 3);
    EGUI_TEST_ASSERT_EQUAL_INT(1, test_widget.read_only_mode);
    assert_pressed_cleared(&test_widget);
    egui_view_card_control_set_read_only_mode(EGUI_VIEW_OF(&test_widget), 0);

    seed_pressed_state(&test_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 1);
    egui_view_card_control_set_palette(EGUI_VIEW_OF(&test_widget), EGUI_COLOR_HEX(0x101112), EGUI_COLOR_HEX(0x202122), EGUI_COLOR_HEX(0x303132),
    ...
    egui_view_card_control_set_current_snapshot(EGUI_VIEW_OF(&test_widget), 1);
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));
}

static void test_card_control_metrics_hit_testing_and_regions(void)
{
    ...
    setup_widget(g_snapshots, EGUI_ARRAY_SIZE(g_snapshots));
    layout_widget();
    egui_view_card_control_get_metrics(&test_widget, EGUI_VIEW_OF(&test_widget), &g_snapshots[0], &metrics);

    EGUI_TEST_ASSERT_TRUE(metrics.card_region.size.width > 0);
    EGUI_TEST_ASSERT_TRUE(metrics.title_region.size.width > 0);
    EGUI_TEST_ASSERT_TRUE(metrics.control_region.size.width > 0);
    EGUI_TEST_ASSERT_TRUE(metrics.meta_region.size.height > 0);

    EGUI_TEST_ASSERT_TRUE(get_part_center(EGUI_VIEW_OF(&test_widget), EGUI_VIEW_CARD_CONTROL_PART_CARD, &x, &y));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_resolve_hit(&test_widget, EGUI_VIEW_OF(&test_widget), x, y));

    egui_view_card_control_set_current_snapshot(EGUI_VIEW_OF(&test_widget), 2);
    layout_widget();
    egui_view_card_control_get_metrics(&test_widget, EGUI_VIEW_OF(&test_widget), &g_snapshots[2], &metrics);
    EGUI_TEST_ASSERT_TRUE(metrics.control_region.size.width > 0);

    egui_view_card_control_set_compact_mode(EGUI_VIEW_OF(&test_widget), 1);
    layout_widget();
    egui_view_card_control_get_metrics(&test_widget, EGUI_VIEW_OF(&test_widget), &g_snapshots[2], &metrics);
    EGUI_TEST_ASSERT_EQUAL_INT(0, metrics.meta_region.size.height);
    EGUI_TEST_ASSERT_TRUE(metrics.body_region.size.height > 0);
}

static void test_card_control_activate_current_part_and_listener(void)
{
    setup_widget(g_snapshots, EGUI_ARRAY_SIZE(g_snapshots));

    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_TRUE(egui_view_card_control_activate_current_part(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(1, g_action_count);
    EGUI_TEST_ASSERT_EQUAL_INT(0, g_action_snapshot);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, g_action_part);

    reset_action_state();
    egui_view_card_control_set_read_only_mode(EGUI_VIEW_OF(&test_widget), 1);
    EGUI_TEST_ASSERT_FALSE(egui_view_card_control_activate_current_part(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, g_action_count);
}

static void test_card_control_touch_same_target_release_and_cancel_behavior(void)
{
    egui_dim_t card_x;
    egui_dim_t card_y;
    ...
    setup_widget(g_snapshots, EGUI_ARRAY_SIZE(g_snapshots));
    layout_widget();
    EGUI_TEST_ASSERT_TRUE(get_part_center(EGUI_VIEW_OF(&test_widget), EGUI_VIEW_CARD_CONTROL_PART_CARD, &card_x, &card_y));
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, test_widget.pressed_part);
...
static void test_card_control_key_navigation_and_activation(void)
{
    setup_widget(g_snapshots, EGUI_ARRAY_SIZE(g_snapshots));
...
static void test_card_control_read_only_and_disabled_guards(void)
{
    egui_dim_t card_x;
    egui_dim_t card_y;
    ...
    EGUI_TEST_ASSERT_TRUE(get_part_center(EGUI_VIEW_OF(&test_widget), EGUI_VIEW_CARD_CONTROL_PART_CARD, &card_x, &card_y));
    egui_view_card_control_set_current_snapshot(EGUI_VIEW_OF(&test_widget), 2);
    initial_snapshot = egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget));
    initial_part = egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget));
    ...
    assert_pressed_cleared(&test_widget);
    EGUI_TEST_ASSERT_EQUAL_INT(initial_snapshot, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(initial_part, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget)));
    ...
    reset_action_state();
    egui_view_card_control_set_current_snapshot(EGUI_VIEW_OF(&test_widget), initial_snapshot);
    egui_view_card_control_set_current_part(EGUI_VIEW_OF(&test_widget), initial_part);
    initial_snapshot = egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&test_widget));
    initial_part = egui_view_card_control_get_current_part(EGUI_VIEW_OF(&test_widget));
...
static void test_card_control_static_preview_consumes_input_and_keeps_state(void)
{
    egui_dim_t card_x;
    egui_dim_t card_y;
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_card_control_get_current_snapshot(EGUI_VIEW_OF(&preview_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_CARD_CONTROL_PART_CARD, egui_view_card_control_get_current_part(EGUI_VIEW_OF(&preview_widget)));
    EGUI_TEST_ASSERT_EQUAL_INT(1, preview_widget.compact_mode);
    EGUI_TEST_ASSERT_EQUAL_INT(0, preview_widget.read_only_mode);
    EGUI_TEST_ASSERT_TRUE(get_part_center(EGUI_VIEW_OF(&preview_widget), EGUI_VIEW_CARD_CONTROL_PART_CARD, &card_x, &card_y));

    preview_widget.current_part = EGUI_VIEW_CARD_CONTROL_PART_CARD;
    seed_pressed_state(&preview_widget, EGUI_VIEW_CARD_CONTROL_PART_CARD, 1);
...
static void test_card_control_internal_helpers(void)
{
    ...
    setup_widget(g_snapshots, EGUI_ARRAY_SIZE(g_snapshots));
    egui_view_card_control_set_palette(EGUI_VIEW_OF(&test_widget), EGUI_COLOR_HEX(0x111111), EGUI_COLOR_HEX(0x222222), EGUI_COLOR_HEX(0x333333),
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x666666).full,
                               egui_view_card_control_tone_color(&test_widget, EGUI_VIEW_CARD_CONTROL_TONE_ACCENT).full);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x777777).full,
                               egui_view_card_control_tone_color(&test_widget, EGUI_VIEW_CARD_CONTROL_TONE_SUCCESS).full);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x888888).full,
                               egui_view_card_control_tone_color(&test_widget, EGUI_VIEW_CARD_CONTROL_TONE_WARNING).full);
...
{
    EGUI_TEST_SUITE_BEGIN(card_control);
    EGUI_TEST_RUN(test_card_control_set_snapshots_and_current_part_clamp);
    EGUI_TEST_RUN(test_card_control_setters_clear_pressed_and_update_state);
    EGUI_TEST_RUN(test_card_control_metrics_hit_testing_and_regions);
    EGUI_TEST_RUN(test_card_control_activate_current_part_and_listener);
    EGUI_TEST_RUN(test_card_control_touch_same_target_release_and_cancel_behavior);
    EGUI_TEST_RUN(test_card_control_key_navigation_and_activation);
    EGUI_TEST_RUN(test_card_control_read_only_and_disabled_guards);
    EGUI_TEST_RUN(test_card_control_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_RUN(test_card_control_internal_helpers);
    EGUI_TEST_SUITE_END();
}
