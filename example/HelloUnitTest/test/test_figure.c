...
#include "../../HelloCustomWidgets/layout/figure/egui_view_figure.h"
#include "../../HelloCustomWidgets/layout/figure/egui_view_figure.c"

typedef struct figure_preview_snapshot figure_preview_snapshot_t;
struct figure_preview_snapshot
{
...
static egui_view_figure_t test_control;
static egui_view_label_t test_child;
static egui_view_figure_t preview_control;
...
static void setup_figure(void)
{
    egui_view_figure_init(EGUI_VIEW_OF(&test_control));
    egui_view_set_size(EGUI_VIEW_OF(&test_control), 160, 80);
}

static void setup_preview_control(void)
{
    egui_view_figure_init(EGUI_VIEW_OF(&preview_control));
    egui_view_set_size(EGUI_VIEW_OF(&preview_control), 92, 48);
    setup_child(&preview_child, 42, 16, "Fig");
    egui_view_figure_set_child(EGUI_VIEW_OF(&preview_control), EGUI_VIEW_OF(&preview_child));
    egui_view_figure_apply_compact_style(EGUI_VIEW_OF(&preview_control));
    egui_view_figure_set_text(EGUI_VIEW_OF(&preview_control), "Fit", "wrap", "Next");
    egui_view_set_on_click_listener(EGUI_VIEW_OF(&preview_control), on_preview_click);
    egui_view_figure_override_static_preview_api(EGUI_VIEW_OF(&preview_control), &preview_api);
    click_count = 0;
}
...
static void capture_preview_snapshot(figure_preview_snapshot_t *snapshot)
{
    ...
    snapshot->figure_surface_color = preview_control.figure_surface_color;
    snapshot->figure_border_color = preview_control.figure_border_color;
    snapshot->accent_color = preview_control.accent_color;
    egui_view_figure_copy_text(snapshot->leading_text, sizeof(snapshot->leading_text), preview_control.leading_text);
    egui_view_figure_copy_text(snapshot->wrap_text, sizeof(snapshot->wrap_text), preview_control.wrap_text);
    egui_view_figure_copy_text(snapshot->trailing_text, sizeof(snapshot->trailing_text), preview_control.trailing_text);
    snapshot->figure_width = preview_control.figure_width;
    snapshot->figure_height = preview_control.figure_height;
    snapshot->wrap_gap = preview_control.wrap_gap;
    snapshot->corner_radius = preview_control.corner_radius;
    snapshot->anchor = preview_control.anchor;
...
static void assert_preview_state_unchanged(const figure_preview_snapshot_t *snapshot)
{
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->figure_surface_color.full, preview_control.figure_surface_color.full);
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->figure_border_color.full, preview_control.figure_border_color.full);
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->figure_width, preview_control.figure_width);
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->figure_height, preview_control.figure_height);
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(snapshot->anchor, preview_control.anchor);
...
static void test_figure_init_defaults(void)
{
    setup_figure();

    EGUI_TEST_ASSERT_TRUE(egui_view_figure_get_child(EGUI_VIEW_OF(&test_control)) == NULL);
    assert_string_equal("Intro text", egui_view_figure_get_leading_text(EGUI_VIEW_OF(&test_control)));
    assert_string_equal("wrapped context", egui_view_figure_get_wrap_text(EGUI_VIEW_OF(&test_control)));
    assert_string_equal("after figure", egui_view_figure_get_trailing_text(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_TRUE(test_control.text_font == (const egui_font_t *)EGUI_CONFIG_FONT_DEFAULT);
    EGUI_TEST_ASSERT_EQUAL_INT(68, egui_view_figure_get_figure_width(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(34, egui_view_figure_get_figure_height(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(5, egui_view_figure_get_wrap_gap(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(8, egui_view_figure_get_corner_radius(EGUI_VIEW_OF(&test_control)));
...
static void test_figure_child_regions_and_layout(void)
{
    ...
    egui_view_figure_set_child(EGUI_VIEW_OF(&test_control), EGUI_VIEW_OF(&test_child));
    EGUI_TEST_ASSERT_TRUE(egui_view_figure_get_child(EGUI_VIEW_OF(&test_control)) == EGUI_VIEW_OF(&test_child));
    ...
    egui_view_figure_layout_child(EGUI_VIEW_OF(&test_control));
    egui_view_figure_get_regions(EGUI_VIEW_OF(&test_control), &leading_region, &figure_region, &wrap_region, &trailing_region);
    EGUI_TEST_ASSERT_EQUAL_INT(12, leading_region.size.height);
    EGUI_TEST_ASSERT_EQUAL_INT(0, figure_region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(17, figure_region.location.y);
    EGUI_TEST_ASSERT_EQUAL_INT(68, figure_region.size.width);
    ...
    egui_view_figure_set_anchor(EGUI_VIEW_OF(&test_control), EGUI_VIEW_FIGURE_ANCHOR_RIGHT);
    egui_view_figure_get_regions(EGUI_VIEW_OF(&test_control), NULL, &figure_region, &wrap_region, NULL);
    EGUI_TEST_ASSERT_EQUAL_INT(92, figure_region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(0, wrap_region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(87, wrap_region.size.width);

    egui_view_figure_set_metrics(EGUI_VIEW_OF(&test_control), 56, 26, 4, 6);
    egui_view_figure_set_compact_mode(EGUI_VIEW_OF(&test_control), 1);
    egui_view_figure_set_anchor(EGUI_VIEW_OF(&test_control), EGUI_VIEW_FIGURE_ANCHOR_CENTER);
    egui_view_figure_get_regions(EGUI_VIEW_OF(&test_control), &leading_region, &figure_region, &wrap_region, NULL);
    EGUI_TEST_ASSERT_EQUAL_INT(10, leading_region.size.height);
    EGUI_TEST_ASSERT_EQUAL_INT(52, figure_region.location.x);
    EGUI_TEST_ASSERT_EQUAL_INT(14, figure_region.location.y);
    EGUI_TEST_ASSERT_EQUAL_INT(44, wrap_region.location.y);
    EGUI_TEST_ASSERT_EQUAL_INT(22, wrap_region.size.height);
}

static void test_figure_styles_palette_text_and_clamps(void)
{
    char long_text[] = "This text is intentionally longer than figure storage";

    setup_figure();
    egui_view_set_pressed(EGUI_VIEW_OF(&test_control), 1);
    egui_view_figure_set_text(EGUI_VIEW_OF(&test_control), long_text, "wrap", "tail");
    EGUI_TEST_ASSERT_FALSE(EGUI_VIEW_OF(&test_control)->is_pressed);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_FIGURE_MAX_TEXT_LEN, (int)strlen(test_control.leading_text));
    assert_string_equal("wrap", test_control.wrap_text);
    assert_string_equal("tail", test_control.trailing_text);

    egui_view_figure_set_metrics(EGUI_VIEW_OF(&test_control), -1, -1, 99, 99);
    EGUI_TEST_ASSERT_EQUAL_INT(42, egui_view_figure_get_figure_width(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(22, egui_view_figure_get_figure_height(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(12, egui_view_figure_get_wrap_gap(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(16, egui_view_figure_get_corner_radius(EGUI_VIEW_OF(&test_control)));

    egui_view_figure_set_metrics(EGUI_VIEW_OF(&test_control), 999, 999, -1, -1);
    EGUI_TEST_ASSERT_EQUAL_INT(118, egui_view_figure_get_figure_width(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(58, egui_view_figure_get_figure_height(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_figure_get_wrap_gap(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_figure_get_corner_radius(EGUI_VIEW_OF(&test_control)));

    egui_view_figure_set_anchor(EGUI_VIEW_OF(&test_control), 99);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_FIGURE_ANCHOR_LEFT, egui_view_figure_get_anchor(EGUI_VIEW_OF(&test_control)));

    egui_view_figure_apply_accent_style(EGUI_VIEW_OF(&test_control));
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_VIEW_FIGURE_ANCHOR_RIGHT, egui_view_figure_get_anchor(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_figure_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(0, egui_view_figure_get_read_only_mode(EGUI_VIEW_OF(&test_control)));

    egui_view_figure_apply_compact_style(EGUI_VIEW_OF(&test_control));
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_figure_get_compact_mode(EGUI_VIEW_OF(&test_control)));
    EGUI_TEST_ASSERT_EQUAL_INT(1, egui_view_figure_get_read_only_mode(EGUI_VIEW_OF(&test_control)));

    egui_view_figure_set_font(EGUI_VIEW_OF(&test_control), (const egui_font_t *)&egui_res_font_montserrat_10_4);
    EGUI_TEST_ASSERT_TRUE(test_control.text_font == (const egui_font_t *)&egui_res_font_montserrat_10_4);

    egui_view_figure_set_palette(EGUI_VIEW_OF(&test_control), EGUI_COLOR_HEX(0x010203), EGUI_COLOR_HEX(0x111213), EGUI_COLOR_HEX(0x212223),
    ...
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x313233).full, test_control.figure_surface_color.full);
    EGUI_TEST_ASSERT_EQUAL_INT(EGUI_COLOR_HEX(0x515253).full, test_control.accent_color.full);
}

static void test_figure_static_preview_consumes_input_and_keeps_state(void)
{
    figure_preview_snapshot_t initial_snapshot;
    egui_dim_t center_x;
    egui_dim_t center_y;

    setup_preview_control();
    layout_view(EGUI_VIEW_OF(&preview_control), 12, 18, 92, 48);
    egui_view_figure_layout_child(EGUI_VIEW_OF(&preview_control));
...
void test_figure_run(void)
{
    EGUI_TEST_SUITE_BEGIN(figure);
    EGUI_TEST_RUN(test_figure_init_defaults);
    EGUI_TEST_RUN(test_figure_child_regions_and_layout);
    EGUI_TEST_RUN(test_figure_styles_palette_text_and_clamps);
    EGUI_TEST_RUN(test_figure_static_preview_consumes_input_and_keeps_state);
    EGUI_TEST_SUITE_END();
}
