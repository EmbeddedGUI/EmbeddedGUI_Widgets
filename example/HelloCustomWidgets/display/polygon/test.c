...
#define POLYGON_ROOT_WIDTH        238
#define POLYGON_ROOT_HEIGHT       184
#define POLYGON_PRIMARY_WIDTH     118
#define POLYGON_PRIMARY_HEIGHT    70
#define POLYGON_PREVIEW_WIDTH     68
#define POLYGON_PREVIEW_HEIGHT    38
#define POLYGON_PREVIEW_ROW_WIDTH 154
#define POLYGON_RECORD_WAIT       90
#define POLYGON_RECORD_FRAME_WAIT 170
#define POLYGON_RECORD_FINAL_WAIT 280
#define POLYGON_DEFAULT_SNAPSHOT  0

#define PRIMARY_SNAPSHOT_COUNT ((uint8_t)EGUI_ARRAY_SIZE(primary_snapshots))

typedef struct polygon_snapshot polygon_snapshot_t;
struct polygon_snapshot
{
    const char *caption;
...
{
    const polygon_snapshot_t *snapshot = &primary_snapshots[index % PRIMARY_SNAPSHOT_COUNT];

    apply_polygon_style(EGUI_VIEW_OF(&primary_control), snapshot->style);
...
{
    apply_primary_snapshot(POLYGON_DEFAULT_SNAPSHOT);
}
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), POLYGON_ROOT_WIDTH, POLYGON_ROOT_HEIGHT);
    ...
    egui_view_polygon_init(EGUI_VIEW_OF(&primary_control));
    egui_view_set_size(EGUI_VIEW_OF(&primary_control), POLYGON_PRIMARY_WIDTH, POLYGON_PRIMARY_HEIGHT);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), POLYGON_PREVIEW_ROW_WIDTH, POLYGON_PREVIEW_HEIGHT);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        EGUI_SIM_SET_WAIT(p_action, POLYGON_RECORD_FRAME_WAIT);
        return true;
    case 1:
        if (first_call)
        {
        ...
        EGUI_SIM_SET_WAIT(p_action, POLYGON_RECORD_WAIT);
        return true;
    case 2:
        if (first_call)
        {
    ...
    case 7:
        ...
        EGUI_SIM_SET_WAIT(p_action, POLYGON_RECORD_FINAL_WAIT);
        return true;
    default:
        break;
    }
