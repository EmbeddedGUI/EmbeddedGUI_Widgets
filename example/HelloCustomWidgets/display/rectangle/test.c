...
#define RECTANGLE_ROOT_WIDTH        238
#define RECTANGLE_ROOT_HEIGHT       180
#define RECTANGLE_PRIMARY_WIDTH     144
#define RECTANGLE_PRIMARY_HEIGHT    58
#define RECTANGLE_PREVIEW_WIDTH     72
#define RECTANGLE_PREVIEW_HEIGHT    34
#define RECTANGLE_PREVIEW_ROW_WIDTH 164
#define RECTANGLE_RECORD_WAIT       90
#define RECTANGLE_RECORD_FRAME_WAIT 170
#define RECTANGLE_RECORD_FINAL_WAIT 280
#define RECTANGLE_DEFAULT_SNAPSHOT  0

#define PRIMARY_SNAPSHOT_COUNT ((uint8_t)EGUI_ARRAY_SIZE(primary_snapshots))

typedef struct rectangle_snapshot rectangle_snapshot_t;
struct rectangle_snapshot
{
    const char *caption;
...
static egui_view_rectangle_t primary_control;
static egui_view_label_t caption_label;
static egui_view_linearlayout_t bottom_row;
static egui_view_rectangle_t thin_preview;
static egui_view_rectangle_t muted_preview;
...
static void apply_rectangle_style(egui_view_t *view, uint8_t style)
{
    switch (style)
    {
    case 1:
        egui_view_rectangle_apply_accent_style(view);
        break;
    case 2:
        apply_rectangle_thin_style(view);
        break;
    case 3:
        apply_rectangle_muted_style(view);
        break;
    default:
        egui_view_rectangle_apply_standard_style(view);
        break;
    }
}
...
{
    const rectangle_snapshot_t *snapshot = &primary_snapshots[index % PRIMARY_SNAPSHOT_COUNT];

    apply_rectangle_style(EGUI_VIEW_OF(&primary_control), snapshot->style);
...
{
    apply_primary_snapshot(RECTANGLE_DEFAULT_SNAPSHOT);
}

static void apply_preview_states(void)
{
    apply_rectangle_thin_style(EGUI_VIEW_OF(&thin_preview));
    apply_rectangle_muted_style(EGUI_VIEW_OF(&muted_preview));
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), RECTANGLE_ROOT_WIDTH, RECTANGLE_ROOT_HEIGHT);
    ...
    egui_view_rectangle_init(EGUI_VIEW_OF(&primary_control));
    egui_view_set_size(EGUI_VIEW_OF(&primary_control), RECTANGLE_PRIMARY_WIDTH, RECTANGLE_PRIMARY_HEIGHT);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), RECTANGLE_PREVIEW_ROW_WIDTH, RECTANGLE_PREVIEW_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&bottom_row), 1);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&bottom_row), EGUI_ALIGN_VCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&bottom_row));

    egui_view_rectangle_init(EGUI_VIEW_OF(&thin_preview));
    egui_view_set_size(EGUI_VIEW_OF(&thin_preview), RECTANGLE_PREVIEW_WIDTH, RECTANGLE_PREVIEW_HEIGHT);
    egui_view_rectangle_override_static_preview_api(EGUI_VIEW_OF(&thin_preview), &thin_preview_api);
    ...
    egui_view_rectangle_init(EGUI_VIEW_OF(&muted_preview));
    egui_view_set_size(EGUI_VIEW_OF(&muted_preview), RECTANGLE_PREVIEW_WIDTH, RECTANGLE_PREVIEW_HEIGHT);
    egui_view_set_margin(EGUI_VIEW_OF(&muted_preview), 16, 0, 0, 0);
    egui_view_rectangle_override_static_preview_api(EGUI_VIEW_OF(&muted_preview), &muted_preview_api);
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        EGUI_SIM_SET_WAIT(p_action, RECTANGLE_RECORD_FRAME_WAIT);
        return true;
    case 1:
        if (first_call)
        {
        ...
        EGUI_SIM_SET_WAIT(p_action, RECTANGLE_RECORD_WAIT);
        return true;
    case 2:
        if (first_call)
        {
    ...
    case 7:
        ...
        EGUI_SIM_SET_WAIT(p_action, RECTANGLE_RECORD_FINAL_WAIT);
        return true;
    default:
        break;
    }
