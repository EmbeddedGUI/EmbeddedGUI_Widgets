...
- 补充对照实现：`ModernWpf`
- 对应组件：`BadgeGroup`
- 当前保留形态：`standard`、`secondary`、`muted`、`accent`、`success`、`warning`、`neutral`
- 当前保留交互：主区保留程序化 snapshot 切换；主控件继续保留 same-target release 与键盘 click 语义；底部 `secondary / muted` 统一保持静态 preview 并吞掉输入
- 当前移除内容：控件本体内置密度 / 只读模式字段与 setter、preview 点击收尾、页面级 `guide / status` chrome、旧双列包裹壳与额外录制桥接帧
...
- 左侧 preview：`secondary`，固定显示 `SET / Small`
- 右侧 preview：`muted`，固定显示 `ARCHIVE / Muted`
- 页面结构：标题 -> 主 `badge_group` -> 底部 `secondary / muted`
...
`example/HelloUnitTest/test/test_badge_group.c` 当前覆盖 `7` 条用例：
...
6. `static preview consumes input and keeps state`
   通过 `badge_group_preview_snapshot_t`、`capture_preview_snapshot()` 与 `assert_preview_state_unchanged()` 固定校验 `region_screen`、`background`、`snapshots`、`font`、`meta_font`、`surface_color`、`border_color`、`text_color`、`muted_text_color`、`accent_color`、`success_color`、`warning_color`、`neutral_color`、`snapshot_count`、`current_snapshot`、`alpha`、`enable`、`is_focused`、`is_pressed` 与 `padding` 不变。
7. `internal helpers cover focus tone text and width`
   覆盖 tone 颜色选择、focus index、防御式文本长度与 pill 宽度估算等内部 helper。

补充说明：

- 主控件继续保留 same-target release 与键盘 click 语义，但不再让底部 preview 参与主区焦点收尾。
- 底部 `secondary / muted` preview 统一通过 `egui_view_badge_group_override_static_preview_api()` 吞掉 `touch / key`，只承担静态 reference 对照职责。
- `Small` 由 APP 侧尺寸和字体触发 dense 布局；`Muted` 由 APP 侧 palette 与 `egui_view_set_enable(..., 0)` 配置，不再由控件本体维护独立模式字段。
...
- 底部 preview 全程静态，不再轮换状态，也不再承担点击桥接职责。

## 9. 验收命令

```bash
make all APP=HelloCustomWidgets APP_SUB=display/badge_group PORT=pc COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0

make clean APP=HelloUnitTest PORT=pc_test COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0
...
make all APP=HelloCustomWidgets APP_SUB=display/badge_group PORT=emscripten COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0
git diff --check
```
...
- `set_snapshots()`、`snapshot / disabled` 切换链路，以及 `touch / key` guard 不能残留 `pressed`。
...
- 保留的交互与实现约束：focus badge 驱动 footer summary、主控件 same-target release 与键盘 click、`snapshot / disabled` 切换清理 `pressed`、static preview 输入抑制
- 删减的旧桥接与装饰：控件本体内置密度 / 只读模式、preview 点击收尾、页面 `guide / status` chrome、旧双列包裹壳与额外录制桥接帧

## 14. 当前验收结果（2026-05-01）

- 单控件编译：已通过 `make all APP=HelloCustomWidgets APP_SUB=display/badge_group PORT=pc COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0`
- `HelloUnitTest`：已通过 `make all APP=HelloUnitTest PORT=pc_test COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0` 和 `.\output\main.exe`，总计 `1048 / 1048`，其中 `badge_group` suite `7 / 7`
...
- 单控件 runtime：已通过 `python scripts/code_runtime_check.py --app HelloCustomWidgets --app-sub display/badge_group --timeout 10 --keep-screenshots`，输出 `11 frames captured -> D:\workspace\gitee\EmbeddedGUI_Widgets\runtime_check_output\HelloCustomWidgets_display_badge_group\default`
- wasm 构建：已通过 `make all APP=HelloCustomWidgets APP_SUB=display/badge_group PORT=emscripten COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0`
- `git diff --check`：已通过，只有 Windows 工作区 LF/CRLF 提示
- 截图复核结论：主区应完整覆盖 `TRIAGE / QUEUE / RISK / CALM` 四组 reference 快照，最终稳定帧回到默认 `TRIAGE`，底部 `secondary / muted` preview 全程静态
