                                           ...
                                           uint8_t index)
{
    ...
    egui_dim_t dot_size = EGUI_VIEW_STATUS_BAR_DOT_SIZE;
    egui_dim_t text_x;
    ...
    if (item_region.size.width < 64 || !egui_view_status_bar_has_text(item->label))
    {
...
static void egui_view_status_bar_on_draw(egui_view_t *self)
{
    ...
    egui_color_t accent = local->accent_color;
    uint8_t index;
    ...
    if (!egui_view_get_enable(self))
    {
        ...
        accent = egui_view_status_bar_mix_disabled(accent);
    }
