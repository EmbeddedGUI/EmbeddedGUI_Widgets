...
- EGUI 适配说明：继续复用 custom 层 `egui_view_font_icon`，当前只收口 `Search / MS24`、`Favorite / MS20`、`Settings / MS16` 三组 reference 快照，不修改 `sdk/EmbeddedGUI`
...
| --- | --- | --- | --- |
| 默认显示 | `Search / MS24` | `Search / MS16` + `subtle` | `Favorite / MS20` + `accent` |
| 快照 2 | `Favorite / MS20` | 保持不变 | 保持不变 |
| 快照 3 | `Settings / MS16` | 保持不变 | 保持不变 |
| 录制最终稳定帧 | `Search / MS24` | 保持不变 | 保持不变 |
| 静态 preview 对照 | 否 | 是 | 是 |
...
当前 `test.c` 已对齐统一的 `ui_ready + layout_page + request_page_snapshot` 收口模板：保留既有 `FONT_ICON_DEFAULT_SNAPSHOT` 与 `apply_primary_default_state()`，初始化阶段在 root view 挂载前后各重放一次默认态与 preview，`case 0` 和最终稳定帧前的默认态恢复统一走显式布局路径。
...
- `HelloCustomWidgets` 单控件编译：`PASS`，`make all APP=HelloCustomWidgets APP_SUB=display/font_icon PORT=pc`
- `HelloUnitTest`：`PASS`，已通过 `make clean APP=HelloUnitTest PORT=pc_test`、`make all APP=HelloUnitTest PORT=pc_test` 与 `X:\output\main.exe`，总计 `845 / 845`，其中 `font_icon` suite `3 / 3`
...
- 单控件 runtime：`PASS`，`9 frames captured -> runtime_check_output/HelloCustomWidgets_display_font_icon/default`
- display 分类 compile/runtime 回归：`PASS`
  compile `21 / 21`，runtime `21 / 21`
- wasm 构建：`PASS`，`web/demos/HelloCustomWidgets_display_font_icon`
- web smoke：`PASS status=Running canvas=480x480 ratio=0.1159 colors=92`
...
- 按主区裁剪后，主区唯一状态数：`3`
- 按 `y >= 256` 裁切底部 preview 区域后，preview 区唯一哈希数：`1`
...
结论：
- 主区变化严格收敛在 `font_icon` 主体，主区外页面 chrome 在整条轨道中保持静态。
- `9` 帧里主区保持 `3` 组唯一状态，对应 `Search / MS24`、`Favorite / MS20`、`Settings / MS16` 三组主区快照；最终稳定帧已显式回到默认 `Search / MS24`。
- 按 `y >= 256` 裁切底部 preview 区域后保持单哈希，确认 `subtle / accent` preview 在整条录制轨道中始终静态一致。

## 12. 已知限制
- 当前 demo 为了确定性固定使用 `Material Symbols` 子集做 reference 展示。
- 当前只覆盖 `Search / MS24`、`Favorite / MS20`、`Settings / MS16` 三组主区快照。
- 当前只收口最小 `FontIcon` 语义，不扩展动画、hover 过渡或组合容器语义。
...
- 当前优先保证主区 3 组 reference 快照、底部 preview 全程静态，以及 runtime 录制稳定，再评估是否需要扩展更多字形或字体来源。
