#include "egui.h"
#include "egui_view_person_picture.h"
#include "uicode.h"
#include "demo_scaffold.h"

#if EGUI_CONFIG_RECORDING_TEST
#include "core/egui_input_simulator.h"
#endif

#define PERSON_PICTURE_ROOT_WIDTH        224
#define PERSON_PICTURE_ROOT_HEIGHT       256
#define PERSON_PICTURE_PANEL_WIDTH       196
#define PERSON_PICTURE_PANEL_HEIGHT      136
#define PERSON_PICTURE_PRIMARY_SIZE      72
#define PERSON_PICTURE_PREVIEW_WIDTH     104
#define PERSON_PICTURE_PREVIEW_HEIGHT    84
#define PERSON_PICTURE_PREVIEW_SIZE      38
#define PERSON_PICTURE_BOTTOM_ROW_WIDTH  216
#define PERSON_PICTURE_BOTTOM_ROW_HEIGHT 84
#define PERSON_PICTURE_RECORD_WAIT       90
#define PERSON_PICTURE_RECORD_FRAME_WAIT 170

typedef struct
{
    const char *heading;
    const char *display_name;
    const char *initials;
    const char *fallback_glyph;
    uint8_t tone;
    uint8_t presence;
    const char *summary;
    egui_color_t summary_color;
    const char *note;
} person_picture_snapshot_t;

static egui_view_linearlayout_t root_layout;
static egui_view_label_t title_label;
static egui_view_linearlayout_t primary_panel;
static egui_view_label_t primary_heading_label;
static egui_view_person_picture_t primary_picture;
static egui_view_label_t primary_summary_label;
static egui_view_label_t primary_note_label;
static egui_view_linearlayout_t bottom_row;
static egui_view_linearlayout_t compact_panel;
static egui_view_label_t compact_heading_label;
static egui_view_person_picture_t compact_picture;
static egui_view_label_t compact_body_label;
static egui_view_linearlayout_t read_only_panel;
static egui_view_label_t read_only_heading_label;
static egui_view_person_picture_t read_only_picture;
static egui_view_label_t read_only_body_label;
static egui_view_api_t compact_picture_api;
static egui_view_api_t read_only_picture_api;

EGUI_BACKGROUND_COLOR_PARAM_INIT_ROUND_RECTANGLE(bg_page_panel_param, EGUI_COLOR_HEX(0xF5F7F9), EGUI_ALPHA_100, 14);
EGUI_BACKGROUND_PARAM_INIT(bg_page_panel_params, &bg_page_panel_param, NULL, NULL);
EGUI_BACKGROUND_COLOR_STATIC_CONST_INIT(bg_page_panel, &bg_page_panel_params);

EGUI_BACKGROUND_COLOR_PARAM_INIT_ROUND_RECTANGLE(bg_surface_panel_param, EGUI_COLOR_HEX(0xFFFFFF), EGUI_ALPHA_100, 12);
EGUI_BACKGROUND_PARAM_INIT(bg_surface_panel_params, &bg_surface_panel_param, NULL, NULL);
EGUI_BACKGROUND_COLOR_STATIC_CONST_INIT(bg_surface_panel, &bg_surface_panel_params);

EGUI_BACKGROUND_COLOR_PARAM_INIT_ROUND_RECTANGLE(bg_muted_panel_param, EGUI_COLOR_HEX(0xFBFCFD), EGUI_ALPHA_100, 12);
EGUI_BACKGROUND_PARAM_INIT(bg_muted_panel_params, &bg_muted_panel_param, NULL, NULL);
EGUI_BACKGROUND_COLOR_STATIC_CONST_INIT(bg_muted_panel, &bg_muted_panel_params);

static const char *title_text = "PersonPicture";

static const person_picture_snapshot_t primary_snapshots[] = {
        {
                "Derived initials",
                "Lena Marsh",
                NULL,
                EGUI_ICON_MS_PERSON,
                EGUI_VIEW_PERSON_PICTURE_TONE_ACCENT,
                EGUI_VIEW_PERSON_PICTURE_PRESENCE_LIVE,
                "LM / available",
                EGUI_COLOR_HEX(0x0F6CBD),
                "Display name resolves a stable monogram.",
        },
        {
                "Manual initials",
                "Aria Rowan",
                "AR",
                EGUI_ICON_MS_PERSON,
                EGUI_VIEW_PERSON_PICTURE_TONE_SUCCESS,
                EGUI_VIEW_PERSON_PICTURE_PRESENCE_BUSY,
                "AR / busy",
                EGUI_COLOR_HEX(0x0F7B45),
                "Explicit initials stay fixed in compact layouts.",
        },
        {
                "Fallback glyph",
                NULL,
                NULL,
                EGUI_ICON_MS_PERSON,
                EGUI_VIEW_PERSON_PICTURE_TONE_NEUTRAL,
                EGUI_VIEW_PERSON_PICTURE_PRESENCE_NONE,
                "Fallback / empty slot",
                EGUI_COLOR_HEX(0x6B7A89),
                "Without name or image, the avatar keeps a readable person glyph.",
        },
};

static void init_text_label(egui_view_label_t *label, egui_dim_t width, egui_dim_t height, const char *text, const egui_font_t *font, egui_color_t color,
                            uint8_t align)
{
    egui_view_label_init(EGUI_VIEW_OF(label));
    egui_view_set_size(EGUI_VIEW_OF(label), width, height);
    egui_view_label_set_text(EGUI_VIEW_OF(label), text);
    egui_view_label_set_align_type(EGUI_VIEW_OF(label), align);
    egui_view_label_set_font(EGUI_VIEW_OF(label), font);
    egui_view_label_set_font_color(EGUI_VIEW_OF(label), color, EGUI_ALPHA_100);
}

static void init_panel(egui_view_linearlayout_t *panel, egui_dim_t width, egui_dim_t height, egui_background_t *background, uint8_t align)
{
    egui_view_linearlayout_init(EGUI_VIEW_OF(panel));
    egui_view_set_size(EGUI_VIEW_OF(panel), width, height);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(panel), 0);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(panel), align);
    egui_view_set_background(EGUI_VIEW_OF(panel), background);
    egui_view_set_padding(EGUI_VIEW_OF(panel), 8, 8, 8, 8);
}

static void apply_picture_state(egui_view_person_picture_t *picture, const person_picture_snapshot_t *snapshot)
{
    egui_view_person_picture_set_display_name(EGUI_VIEW_OF(picture), snapshot->display_name);
    egui_view_person_picture_set_initials(EGUI_VIEW_OF(picture), snapshot->initials);
    egui_view_person_picture_set_fallback_glyph(EGUI_VIEW_OF(picture), snapshot->fallback_glyph);
    egui_view_person_picture_set_image(EGUI_VIEW_OF(picture), NULL);
    egui_view_person_picture_set_tone(EGUI_VIEW_OF(picture), snapshot->tone);
    egui_view_person_picture_set_presence(EGUI_VIEW_OF(picture), snapshot->presence);
}

static void apply_primary_snapshot(uint8_t index)
{
    const person_picture_snapshot_t *snapshot = &primary_snapshots[index % EGUI_ARRAY_SIZE(primary_snapshots)];

    apply_picture_state(&primary_picture, snapshot);
    egui_view_label_set_text(EGUI_VIEW_OF(&primary_heading_label), snapshot->heading);
    egui_view_label_set_text(EGUI_VIEW_OF(&primary_summary_label), snapshot->summary);
    egui_view_label_set_font_color(EGUI_VIEW_OF(&primary_summary_label), snapshot->summary_color, EGUI_ALPHA_100);
    egui_view_label_set_text(EGUI_VIEW_OF(&primary_note_label), snapshot->note);
}

static void apply_preview_states(void)
{
    static const person_picture_snapshot_t compact_snapshot = {
            "Compact",
            "Maya Yu",
            NULL,
            EGUI_ICON_MS_PERSON,
            EGUI_VIEW_PERSON_PICTURE_TONE_WARNING,
            EGUI_VIEW_PERSON_PICTURE_PRESENCE_AWAY,
            "Compact",
            EGUI_COLOR_HEX(0x9D5D00),
            "Monogram",
    };
    static const person_picture_snapshot_t read_only_snapshot = {
            "Read only",
            "Mina Brooks",
            "MB",
            EGUI_ICON_MS_PERSON,
            EGUI_VIEW_PERSON_PICTURE_TONE_NEUTRAL,
            EGUI_VIEW_PERSON_PICTURE_PRESENCE_OFFLINE,
            "Muted",
            EGUI_COLOR_HEX(0x7A8796),
            "Read only",
    };

    apply_picture_state(&compact_picture, &compact_snapshot);
    apply_picture_state(&read_only_picture, &read_only_snapshot);
    egui_view_person_picture_set_compact_mode(EGUI_VIEW_OF(&compact_picture), 1);
    egui_view_person_picture_set_compact_mode(EGUI_VIEW_OF(&read_only_picture), 1);
    egui_view_person_picture_set_read_only_mode(EGUI_VIEW_OF(&read_only_picture), 1);
}

void test_init_ui(void)
{
    egui_view_linearlayout_init(EGUI_VIEW_OF(&root_layout));
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), PERSON_PICTURE_ROOT_WIDTH, PERSON_PICTURE_ROOT_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&root_layout), 0);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&root_layout), EGUI_ALIGN_HCENTER);
    egui_view_set_background(EGUI_VIEW_OF(&root_layout), EGUI_BG_OF(&bg_page_panel));

    init_text_label(&title_label, PERSON_PICTURE_ROOT_WIDTH, 18, title_text, (const egui_font_t *)&egui_res_font_montserrat_12_4, EGUI_COLOR_HEX(0x21303F),
                    EGUI_ALIGN_CENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&title_label), 0, 8, 0, 6);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&title_label));

    init_panel(&primary_panel, PERSON_PICTURE_PANEL_WIDTH, PERSON_PICTURE_PANEL_HEIGHT, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&primary_panel), 0, 0, 0, 10);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&primary_panel));

    init_text_label(&primary_heading_label, 176, 12, "Derived initials", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x51606F),
                    EGUI_ALIGN_CENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&primary_heading_label), 0, 0, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&primary_panel), EGUI_VIEW_OF(&primary_heading_label));

    egui_view_person_picture_init(EGUI_VIEW_OF(&primary_picture));
    egui_view_set_size(EGUI_VIEW_OF(&primary_picture), PERSON_PICTURE_PRIMARY_SIZE, PERSON_PICTURE_PRIMARY_SIZE);
    egui_view_set_margin(EGUI_VIEW_OF(&primary_picture), 0, 0, 0, 6);
    egui_view_person_picture_set_font(EGUI_VIEW_OF(&primary_picture), (const egui_font_t *)&egui_res_font_montserrat_12_4);
    egui_view_group_add_child(EGUI_VIEW_OF(&primary_panel), EGUI_VIEW_OF(&primary_picture));

    init_text_label(&primary_summary_label, 176, 12, "LM / available", (const egui_font_t *)&egui_res_font_montserrat_10_4, EGUI_COLOR_HEX(0x0F6CBD),
                    EGUI_ALIGN_CENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&primary_summary_label), 0, 0, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&primary_panel), EGUI_VIEW_OF(&primary_summary_label));

    init_text_label(&primary_note_label, 176, 12, "Display name resolves a stable monogram.", (const egui_font_t *)&egui_res_font_montserrat_8_4,
                    EGUI_COLOR_HEX(0x6B7A89), EGUI_ALIGN_CENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&primary_panel), EGUI_VIEW_OF(&primary_note_label));

    egui_view_linearlayout_init(EGUI_VIEW_OF(&bottom_row));
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), PERSON_PICTURE_BOTTOM_ROW_WIDTH, PERSON_PICTURE_BOTTOM_ROW_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&bottom_row), 1);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&bottom_row), EGUI_ALIGN_VCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&bottom_row));

    init_panel(&compact_panel, PERSON_PICTURE_PREVIEW_WIDTH, PERSON_PICTURE_PREVIEW_HEIGHT, EGUI_BG_OF(&bg_muted_panel), EGUI_ALIGN_HCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&bottom_row), EGUI_VIEW_OF(&compact_panel));

    init_text_label(&compact_heading_label, 84, 12, "Compact", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x233241),
                    EGUI_ALIGN_CENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&compact_heading_label), 0, 0, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_panel), EGUI_VIEW_OF(&compact_heading_label));

    egui_view_person_picture_init(EGUI_VIEW_OF(&compact_picture));
    egui_view_set_size(EGUI_VIEW_OF(&compact_picture), PERSON_PICTURE_PREVIEW_SIZE, PERSON_PICTURE_PREVIEW_SIZE);
    egui_view_set_margin(EGUI_VIEW_OF(&compact_picture), 0, 0, 0, 4);
    egui_view_person_picture_set_font(EGUI_VIEW_OF(&compact_picture), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_person_picture_override_static_preview_api(EGUI_VIEW_OF(&compact_picture), &compact_picture_api);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_panel), EGUI_VIEW_OF(&compact_picture));

    init_text_label(&compact_body_label, 84, 10, "Monogram", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x6B7A89),
                    EGUI_ALIGN_CENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_panel), EGUI_VIEW_OF(&compact_body_label));

    init_panel(&read_only_panel, PERSON_PICTURE_PREVIEW_WIDTH, PERSON_PICTURE_PREVIEW_HEIGHT, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&read_only_panel), 8, 0, 0, 0);
    egui_view_group_add_child(EGUI_VIEW_OF(&bottom_row), EGUI_VIEW_OF(&read_only_panel));

    init_text_label(&read_only_heading_label, 84, 12, "Read only", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x233241),
                    EGUI_ALIGN_CENTER);
    egui_view_set_margin(EGUI_VIEW_OF(&read_only_heading_label), 0, 0, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&read_only_panel), EGUI_VIEW_OF(&read_only_heading_label));

    egui_view_person_picture_init(EGUI_VIEW_OF(&read_only_picture));
    egui_view_set_size(EGUI_VIEW_OF(&read_only_picture), PERSON_PICTURE_PREVIEW_SIZE, PERSON_PICTURE_PREVIEW_SIZE);
    egui_view_set_margin(EGUI_VIEW_OF(&read_only_picture), 0, 0, 0, 4);
    egui_view_person_picture_set_font(EGUI_VIEW_OF(&read_only_picture), (const egui_font_t *)&egui_res_font_montserrat_8_4);
    egui_view_person_picture_set_palette(EGUI_VIEW_OF(&read_only_picture), EGUI_COLOR_HEX(0xFBFCFD), EGUI_COLOR_HEX(0xD8DFE6), EGUI_COLOR_HEX(0xFFFFFF),
                                         EGUI_COLOR_HEX(0xA7B4C1), EGUI_COLOR_HEX(0xB2C4BA), EGUI_COLOR_HEX(0xC4B8A4), EGUI_COLOR_HEX(0xB4BDC8),
                                         EGUI_COLOR_HEX(0x8C98A4));
    egui_view_person_picture_override_static_preview_api(EGUI_VIEW_OF(&read_only_picture), &read_only_picture_api);
    egui_view_group_add_child(EGUI_VIEW_OF(&read_only_panel), EGUI_VIEW_OF(&read_only_picture));

    init_text_label(&read_only_body_label, 84, 10, "Read only", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x6B7A89),
                    EGUI_ALIGN_CENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&read_only_panel), EGUI_VIEW_OF(&read_only_body_label));

    apply_primary_snapshot(0);
    apply_preview_states();

    hello_custom_widgets_demo_apply_title_only_scaffold(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&title_label), NULL, 0);

    egui_view_linearlayout_layout_childs(EGUI_VIEW_OF(&primary_panel));
    egui_view_linearlayout_layout_childs(EGUI_VIEW_OF(&compact_panel));
    egui_view_linearlayout_layout_childs(EGUI_VIEW_OF(&read_only_panel));
    egui_view_linearlayout_layout_childs(EGUI_VIEW_OF(&bottom_row));
    egui_view_linearlayout_layout_childs(EGUI_VIEW_OF(&root_layout));

    egui_core_add_user_root_view(EGUI_VIEW_OF(&root_layout));
    egui_core_layout_childs_user_root_view(EGUI_LAYOUT_VERTICAL, EGUI_ALIGN_HCENTER | EGUI_ALIGN_VCENTER);
}

#if EGUI_CONFIG_RECORDING_TEST
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    static int last_action = -1;
    int first_call = action_index != last_action;

    last_action = action_index;

    switch (action_index)
    {
    case 0:
        if (first_call)
        {
            apply_primary_snapshot(0);
            apply_preview_states();
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_WAIT);
        return true;
    case 1:
        if (first_call)
        {
            recording_request_snapshot();
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_FRAME_WAIT);
        return true;
    case 2:
        if (first_call)
        {
            apply_primary_snapshot(1);
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_WAIT);
        return true;
    case 3:
        if (first_call)
        {
            recording_request_snapshot();
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_FRAME_WAIT);
        return true;
    case 4:
        if (first_call)
        {
            apply_primary_snapshot(2);
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_WAIT);
        return true;
    case 5:
        if (first_call)
        {
            recording_request_snapshot();
        }
        EGUI_SIM_SET_WAIT(p_action, PERSON_PICTURE_RECORD_FRAME_WAIT);
        return true;
    case 6:
        if (first_call)
        {
            apply_primary_snapshot(0);
            recording_request_snapshot();
        }
        EGUI_SIM_SET_WAIT(p_action, 520);
        return true;
    default:
        return false;
    }
}
#endif
