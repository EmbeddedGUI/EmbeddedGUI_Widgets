...
#define TEXT_BLOCK_ROOT_HEIGHT       246
#define TEXT_BLOCK_PANEL_WIDTH       196
#define TEXT_BLOCK_PANEL_HEIGHT      118
#define TEXT_BLOCK_PRIMARY_WIDTH     176
#define TEXT_BLOCK_PRIMARY_HEIGHT    56
#define TEXT_BLOCK_PREVIEW_WIDTH     104
#define TEXT_BLOCK_PREVIEW_HEIGHT    82
#define TEXT_BLOCK_PREVIEW_BODY_W    84
#define TEXT_BLOCK_PREVIEW_BODY_H    34
#define TEXT_BLOCK_BOTTOM_ROW_WIDTH  216
#define TEXT_BLOCK_BOTTOM_ROW_HEIGHT 82
...
static const text_block_snapshot_t primary_snapshots[] = {
        {
                "Standard body",
                "A TextBlock keeps one paragraph readable\nwithout turning it into an editor or a scroller.",
                EGUI_VIEW_TEXT_BLOCK_STYLE_STANDARD,
                "Standard style",
                EGUI_COLOR_HEX(0x22303F),
                "Single-paragraph copy stays calm and direct.",
        },
        {
                "Subtle note",
                "Secondary notes can wrap across two lines\nwhile staying visually quieter than body text.",
                EGUI_VIEW_TEXT_BLOCK_STYLE_SUBTLE,
                "Subtle style",
                EGUI_COLOR_HEX(0x6B7A89),
                "Useful for timestamps, guidance, and low-priority context.",
        },
        {
                "Accent line",
                "Accent text can call out a short action\nwithout promoting the whole block into a card.",
                EGUI_VIEW_TEXT_BLOCK_STYLE_ACCENT,
                "Accent style",
                EGUI_COLOR_HEX(0x0F6CBD),
                "The wrapper only changes typography and tone, not semantics.",
        },
};
...
{
    const text_block_snapshot_t *snapshot = &primary_snapshots[index % EGUI_ARRAY_SIZE(primary_snapshots)];

    egui_view_label_set_text(EGUI_VIEW_OF(&primary_heading_label), snapshot->heading);
    egui_view_text_block_set_text(EGUI_VIEW_OF(&primary_widget), snapshot->body);
...
{
    egui_view_text_block_set_text(EGUI_VIEW_OF(&compact_widget), "Short copy\nfor tighter slots.");
    egui_view_text_block_set_compact_mode(EGUI_VIEW_OF(&compact_widget), 1);
    egui_view_text_block_apply_subtle_style(EGUI_VIEW_OF(&compact_widget));
    egui_view_textblock_set_max_lines(EGUI_VIEW_OF(&compact_widget), 2);
    egui_view_label_set_text(EGUI_VIEW_OF(&compact_body_label), "Compact");
    ...
    egui_view_text_block_apply_standard_style(EGUI_VIEW_OF(&read_only_widget));
    ...
    egui_view_label_set_text(EGUI_VIEW_OF(&read_only_body_label), "Read only");
}
...
void test_init_ui(void)
{
    ...
    init_panel(&primary_panel, TEXT_BLOCK_PANEL_WIDTH, TEXT_BLOCK_PANEL_HEIGHT, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    ...
    init_text_label(&primary_heading_label, 176, 12, "Standard body", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x51606F),
                    EGUI_ALIGN_CENTER);
    ...
    init_text_label(&primary_summary_label, 176, 12, "Standard style", (const egui_font_t *)&egui_res_font_montserrat_10_4, EGUI_COLOR_HEX(0x22303F),
                    EGUI_ALIGN_CENTER);
    ...
    init_text_label(&primary_note_label, 176, 20, "Single-paragraph copy stays calm and direct.",
    ...
    init_panel(&compact_panel, TEXT_BLOCK_PREVIEW_WIDTH, TEXT_BLOCK_PREVIEW_HEIGHT, EGUI_BG_OF(&bg_muted_panel), EGUI_ALIGN_HCENTER);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&compact_widget), TEXT_BLOCK_PREVIEW_BODY_W, TEXT_BLOCK_PREVIEW_BODY_H);
    ...
    init_panel(&read_only_panel, TEXT_BLOCK_PREVIEW_WIDTH, TEXT_BLOCK_PREVIEW_HEIGHT, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&read_only_widget), TEXT_BLOCK_PREVIEW_BODY_W, TEXT_BLOCK_PREVIEW_BODY_H);
