...
- 当前移除内容：旧主 `primary_panel / heading / body / note`、底部 `subtle_panel / accent_panel` 包装与说明文案、录制末尾回切默认态恢复帧
...
| --- | --- | --- | --- |
| `standard` style | 是 | API 保留 | API 保留 |
| `accent` style | 是 | 否 | 是 |
| `subtle` style | 是 | 是 | 否 |
| palette setter | 是 | API 保留 | API 保留 |
| 接收焦点 / 交互 | 否 | 否 | 否 |
...
说明：
- 录制阶段不再回切 `Standard / neutral` 后额外补一帧。
...
## 10. 当前结果
- `HelloCustomWidgets` 单控件编译：已通过 `make all APP=HelloCustomWidgets APP_SUB=display/divider PORT=pc`
- `HelloUnitTest`：已在 `X:\` 短路径通过 `make clean APP=HelloUnitTest PORT=pc_test`、`make all APP=HelloUnitTest PORT=pc_test` 和 `X:\output\main.exe`，总计 `845 / 845`，其中 `divider` suite `3 / 3`
...
- 单控件 runtime：已通过 `python scripts/code_runtime_check.py --app HelloCustomWidgets --app-sub display/divider --track reference --timeout 10 --keep-screenshots`，输出 `8` 帧截图
...
- wasm 构建：已通过 `python scripts/web/wasm_build_demos.py --app HelloCustomWidgets --app-sub display/divider`，输出 `web/demos/HelloCustomWidgets_display_divider`
- web smoke：已通过 `python scripts/web/web_smoke_check.py --web-root web --manifest web/demos/demos.json --demo HelloCustomWidgets_display_divider`，结果 `PASS status=Running canvas=480x480 ratio=0.1034 colors=65`
