...
#define PRESENCE_BADGE_ROOT_HEIGHT       206
#define PRESENCE_BADGE_PANEL_WIDTH       196
#define PRESENCE_BADGE_PANEL_HEIGHT      96
#define PRESENCE_BADGE_PRIMARY_SIZE      28
#define PRESENCE_BADGE_PREVIEW_PANEL_W   104
#define PRESENCE_BADGE_PREVIEW_PANEL_H   68
#define PRESENCE_BADGE_PREVIEW_SIZE      18
#define PRESENCE_BADGE_BOTTOM_ROW_WIDTH  216
#define PRESENCE_BADGE_BOTTOM_ROW_HEIGHT 68
#define PRESENCE_BADGE_RECORD_WAIT       90
#define PRESENCE_BADGE_RECORD_FRAME_WAIT 170
#define PRESENCE_BADGE_RECORD_FINAL_WAIT 520
...
static const presence_badge_snapshot_t primary_snapshots[] = {
        {
                ...
                "Filled green status stays readable on a light surface.",
        ...
        },
        {
                "Do not disturb",
                "Blocked focus time",
                "The minus glyph keeps the blocked state distinct from a plain busy red dot.",
                EGUI_VIEW_PRESENCE_BADGE_STATUS_DO_NOT_DISTURB,
                EGUI_COLOR_HEX(0xC4314B),
        },
        {
                "Offline",
                "Disconnected but visible",
                "Ring-only offline status keeps a low-noise fallback on light cards.",
...
{
    const presence_badge_snapshot_t *snapshot = &primary_snapshots[index % EGUI_ARRAY_SIZE(primary_snapshots)];
...
void test_init_ui(void)
{
    ...
    init_panel(&primary_panel, PRESENCE_BADGE_PANEL_WIDTH, PRESENCE_BADGE_PANEL_HEIGHT, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    ...
    init_text_label(&primary_heading_label, 176, 12, "Available", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x51606F),
                    EGUI_ALIGN_CENTER);
    ...
    init_text_label(&primary_summary_label, 176, 12, "Ready to collaborate", (const egui_font_t *)&egui_res_font_montserrat_10_4, EGUI_COLOR_HEX(0x107C41),
                    EGUI_ALIGN_CENTER);
    ...
    init_text_label(&primary_note_label, 176, 12, "Filled green status stays readable on a light surface.",
    ...
    init_panel(&compact_panel, PRESENCE_BADGE_PREVIEW_PANEL_W, PRESENCE_BADGE_PREVIEW_PANEL_H, EGUI_BG_OF(&bg_muted_panel), EGUI_ALIGN_HCENTER);
    ...
    init_text_label(&compact_note_label, 84, 12, "Away", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x875C00),
                    EGUI_ALIGN_CENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&compact_panel), EGUI_VIEW_OF(&compact_note_label));

    init_panel(&read_only_panel, PRESENCE_BADGE_PREVIEW_PANEL_W, PRESENCE_BADGE_PREVIEW_PANEL_H, EGUI_BG_OF(&bg_surface_panel), EGUI_ALIGN_HCENTER);
    ...
    init_text_label(&read_only_note_label, 84, 12, "Muted busy", (const egui_font_t *)&egui_res_font_montserrat_8_4, EGUI_COLOR_HEX(0x6B7A89),
                    EGUI_ALIGN_CENTER);
