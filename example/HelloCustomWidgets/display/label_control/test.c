...
#define LABEL_CONTROL_ROOT_WIDTH        238
#define LABEL_CONTROL_ROOT_HEIGHT       176
#define LABEL_CONTROL_PRIMARY_WIDTH     176
#define LABEL_CONTROL_PRIMARY_HEIGHT    44
#define LABEL_CONTROL_PREVIEW_WIDTH     92
#define LABEL_CONTROL_PREVIEW_HEIGHT    30
#define LABEL_CONTROL_PREVIEW_ROW_WIDTH 198
#define LABEL_CONTROL_RECORD_WAIT       90
#define LABEL_CONTROL_RECORD_FRAME_WAIT 170
#define LABEL_CONTROL_RECORD_FINAL_WAIT 280
#define LABEL_CONTROL_DEFAULT_SNAPSHOT  0

#define PRIMARY_SNAPSHOT_COUNT ((uint8_t)EGUI_ARRAY_SIZE(primary_snapshots))

typedef struct label_control_snapshot label_control_snapshot_t;
struct label_control_snapshot
{
    const char *text;
    const char *hint;
    const char *caption;
...
{
    const label_control_snapshot_t *snapshot = &primary_snapshots[index % PRIMARY_SNAPSHOT_COUNT];

    apply_label_control_style(EGUI_VIEW_OF(&primary_control), snapshot->style);
    egui_view_label_control_set_text(EGUI_VIEW_OF(&primary_control), snapshot->text);
    egui_view_label_control_set_target_hint(EGUI_VIEW_OF(&primary_control), snapshot->hint);
    egui_view_label_control_set_access_key_index(EGUI_VIEW_OF(&primary_control), snapshot->access_key_index);
    egui_view_label_control_set_required(EGUI_VIEW_OF(&primary_control), snapshot->required);
...
{
    apply_primary_snapshot(LABEL_CONTROL_DEFAULT_SNAPSHOT);
}
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), LABEL_CONTROL_ROOT_WIDTH, LABEL_CONTROL_ROOT_HEIGHT);
    ...
    egui_view_label_control_init(EGUI_VIEW_OF(&primary_control));
    egui_view_set_size(EGUI_VIEW_OF(&primary_control), LABEL_CONTROL_PRIMARY_WIDTH, LABEL_CONTROL_PRIMARY_HEIGHT);
    egui_view_set_margin(EGUI_VIEW_OF(&primary_control), 0, 0, 0, 10);
    egui_view_label_control_set_fonts(EGUI_VIEW_OF(&primary_control), (const egui_font_t *)&egui_res_font_montserrat_10_4,
    ...
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), LABEL_CONTROL_PREVIEW_ROW_WIDTH, LABEL_CONTROL_PREVIEW_HEIGHT);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        EGUI_SIM_SET_WAIT(p_action, LABEL_CONTROL_RECORD_FRAME_WAIT);
        return true;
    case 1:
        if (first_call)
        {
        ...
        EGUI_SIM_SET_WAIT(p_action, LABEL_CONTROL_RECORD_WAIT);
        return true;
    case 2:
        if (first_call)
        {
    ...
    case 8:
        ...
        EGUI_SIM_SET_WAIT(p_action, LABEL_CONTROL_RECORD_FINAL_WAIT);
        return true;
    default:
        return false;
    }
