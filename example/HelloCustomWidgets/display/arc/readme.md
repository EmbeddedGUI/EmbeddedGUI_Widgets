...
## 10. 当前结果
- `HelloCustomWidgets` 单控件编译：已通过 `make all APP=HelloCustomWidgets APP_SUB=display/arc PORT=pc`
- `HelloUnitTest`：已在 `X:\` 短路径通过 `make clean APP=HelloUnitTest PORT=pc_test`、`make all APP=HelloUnitTest PORT=pc_test` 与 `X:\output\main.exe`，总计 `845 / 845`，其中 `arc` suite `3 / 3`
...
- 单控件 runtime：已通过 `python scripts/code_runtime_check.py --app HelloCustomWidgets --app-sub display/arc --track reference --timeout 10 --keep-screenshots`，输出 `8` 帧截图
...
- wasm 构建：已通过 `python scripts/web/wasm_build_demos.py --app HelloCustomWidgets --app-sub display/arc`，输出 `web/demos/HelloCustomWidgets_display_arc`
- web smoke：已通过 `python scripts/web/web_smoke_check.py --web-root web --manifest web/demos/demos.json --demo HelloCustomWidgets_display_arc`，结果 `PASS status=Running canvas=480x480 ratio=0.1488 colors=154`
...
- 按主区裁剪后，主区唯一状态数：`3`
- 按 `y >= 277` 裁剪底部 preview 区域后，preview 区唯一哈希数：`1`
