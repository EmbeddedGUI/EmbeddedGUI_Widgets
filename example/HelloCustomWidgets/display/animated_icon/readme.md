...
## 6. 状态矩阵
| 状态 | 主控件 | Chevron preview | Fallback preview |
| --- | --- | --- | --- |
| 默认显示 | `Normal` | `Pressed` | `Normal` |
| 快照 2 | `PointerOver` | 保持不变 | 保持不变 |
| 快照 3 | `Pressed` | 保持不变 | 保持不变 |
| 录制最终稳定帧 | `Normal` | 保持不变 | 保持不变 |
| 静态 preview 对照 | 否 | 是 | 是 |
...
当前 `test.c` 已对齐统一的 `ui_ready + layout_page + request_page_snapshot` 收口模板：保留既有 `ANIMATED_ICON_DEFAULT_SNAPSHOT` 与 `apply_primary_default_state()`，初始化阶段在 root view 挂载前后各重放一次默认态与 preview，`case 0` 和最终稳定帧前的默认态恢复统一走显式布局路径。
...
- `HelloCustomWidgets` 单控件编译：`PASS`，`make all APP=HelloCustomWidgets APP_SUB=display/animated_icon PORT=pc`
- `HelloUnitTest`：`PASS`，已通过 `make clean APP=HelloUnitTest PORT=pc_test`、`make all APP=HelloUnitTest PORT=pc_test` 与 `X:\output\main.exe`，总计 `845 / 845`，其中 `animated_icon` suite `4 / 4`
...
- 单控件 runtime：`PASS`，`9 frames captured -> runtime_check_output/HelloCustomWidgets_display_animated_icon/default`
- display 分类 compile/runtime 回归：`PASS`
  compile `21 / 21`，runtime `21 / 21`
- wasm 构建：`PASS`，`web/demos/HelloCustomWidgets_display_animated_icon`
- web smoke：`PASS status=Running canvas=480x480 ratio=0.1253 colors=93`
...
结论：
- 主区变化严格收敛在 `animated_icon` 主体，主区外页面 chrome 在整条轨道中保持静态。
- `9` 帧里主区共保持 `6` 组唯一状态：默认 `Normal` 稳定态、`PointerOver` 过渡/稳定态、`Pressed` 过渡/稳定态，以及回到默认 `Normal` 前的收尾过渡帧；最终稳定帧已显式回到默认 `Normal`。
- 按 `y >= 263` 裁切底部 preview 区域后保持单哈希，确认 `chevron / fallback` preview 在整条录制轨道中始终静态一致。
