...
#define EGUI_VIEW_INFO_BAR_PART_CLOSE  2

typedef struct egui_view_info_bar_metrics egui_view_info_bar_metrics_t;
struct egui_view_info_bar_metrics
{
...
static uint8_t egui_view_info_bar_clamp_snapshot_count(uint8_t count)
{
    if (count > EGUI_VIEW_INFO_BAR_MAX_SNAPSHOTS)
    {
        return EGUI_VIEW_INFO_BAR_MAX_SNAPSHOTS;
    }
    return count;
}

static uint8_t egui_view_info_bar_text_len(const char *text)
{
    uint8_t length = 0;
...
static uint8_t egui_view_info_bar_is_space_char(char c)
{
    return (uint8_t)(c == ' ' || c == '\t');
}

static uint8_t egui_view_info_bar_is_break_after_char(char c)
{
    return (uint8_t)(c == '-' || c == '/');
}

static uint8_t egui_view_info_bar_find_elide_boundary(const char *text, uint8_t visible_chars)
{
    uint8_t index;
    ...
    {
        if (egui_view_info_bar_is_space_char(text[index - 1]))
        {
    ...
    for (index = visible_chars; index > 0; --index)
    {
        if (egui_view_info_bar_is_break_after_char(text[index - 1]))
        {
            return index;
        }
    }
...
static void egui_view_info_bar_copy_elided(char *buffer, uint8_t buffer_size, const char *text, uint8_t max_chars)
{
    uint8_t copy_length;
    uint8_t index;
    uint8_t length;
    ...
    length = egui_view_info_bar_text_len(text);
    if (length <= max_chars)
    {
        copy_length = length;
    ...
    copy_length = egui_view_info_bar_find_elide_boundary(text, (uint8_t)(max_chars - 3));
...
static egui_dim_t egui_view_info_bar_measure_text_width(const egui_font_t *font, const char *text)
{
...
static void egui_view_info_bar_fit_text_to_width(const egui_font_t *font, const char *text, char *buffer, uint8_t buffer_size, egui_dim_t max_width,
                                                 egui_dim_t fallback_char_width)
{
    ...
    max_chars = egui_view_info_bar_text_len(text);
    egui_view_info_bar_copy_elided(buffer, buffer_size, text, max_chars);
    while (max_chars > 0)
    {
        egui_dim_t text_width = egui_view_info_bar_measure_text_width(font, buffer);

        if (text_width <= 0)
        {
            text_width = (egui_dim_t)egui_view_info_bar_text_len(buffer) * fallback_char_width;
        }
...
static egui_dim_t egui_view_info_bar_measure_font_line_height(const egui_font_t *font)
{
...
static egui_dim_t egui_view_info_bar_resolve_line_height(const egui_font_t *font, egui_dim_t fallback)
{
    egui_dim_t line_height = egui_view_info_bar_measure_font_line_height(font);
...
static const char *egui_view_info_bar_severity_glyph(uint8_t severity)
{
    switch (severity)
    {
    case 1:
...
static uint8_t egui_view_info_bar_clear_pressed_state(egui_view_t *self, egui_view_info_bar_t *local)
{
    uint8_t had_state = (uint8_t)(self->is_pressed || local->pressed_part != EGUI_VIEW_INFO_BAR_PART_NONE);

    local->pressed_part = EGUI_VIEW_INFO_BAR_PART_NONE;
    if (self->is_pressed)
    {
...
static const egui_view_info_bar_snapshot_t *egui_view_info_bar_get_snapshot(egui_view_info_bar_t *local)
{
...
static uint8_t egui_view_info_bar_snapshot_has_action(const egui_view_info_bar_snapshot_t *snapshot)
{
    return (uint8_t)(snapshot != NULL && snapshot->show_action && snapshot->action != NULL && snapshot->action[0] != '\0');
}

static uint8_t egui_view_info_bar_is_interactive(egui_view_t *self, egui_view_info_bar_t *local)
{
    return (uint8_t)(local->opened && !local->read_only_mode && egui_view_get_enable(self));
}

static void egui_view_info_bar_emit_open_changed(egui_view_t *self, egui_view_info_bar_t *local)
{
...
static void egui_view_info_bar_set_opened_inner(egui_view_t *self, egui_view_info_bar_t *local, uint8_t opened, uint8_t notify)
{
    uint8_t had_pressed = egui_view_info_bar_clear_pressed_state(self, local);
    ...
    if (notify)
    {
        egui_view_info_bar_emit_open_changed(self, local);
    }
}

static void egui_view_info_bar_emit_action(egui_view_t *self, egui_view_info_bar_t *local)
{
...
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
    uint8_t had_pressed = egui_view_info_bar_clear_pressed_state(self, local);

    local->snapshots = snapshots;
    local->snapshot_count = snapshots == NULL ? 0 : egui_view_info_bar_clamp_snapshot_count(snapshot_count);
...
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
    egui_view_info_bar_set_opened_inner(self, local, opened, 0);
}
...
static void egui_view_info_bar_get_metrics(egui_view_info_bar_t *local, egui_view_t *self, egui_view_info_bar_metrics_t *metrics)
{
    const egui_view_info_bar_snapshot_t *snapshot = egui_view_info_bar_get_snapshot(local);
    ...
    metrics->content_region.location.x = region.location.x + pad_x + (local->compact_mode ? 2 : 4);
    ...
    {
        egui_dim_t text_w = egui_view_info_bar_measure_text_width(local->font, snapshot->action);

        if (text_w <= 0)
        {
            text_w = (egui_dim_t)egui_view_info_bar_text_len(snapshot->action) * (local->compact_mode ? 4 : 5);
        }
...
static void egui_view_info_bar_draw_text(const egui_font_t *font, egui_view_t *self, const char *text, const egui_region_t *region, uint8_t align, egui_color_t color)
{
...
static void egui_view_info_bar_draw_close(egui_view_t *self, const egui_region_t *region, egui_color_t color)
{
    egui_dim_t x0;
    egui_dim_t y0;
    egui_dim_t x1;
...
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
    const egui_view_info_bar_snapshot_t *snapshot;
    ...
    egui_view_get_work_region(self, &region);
    snapshot = egui_view_info_bar_get_snapshot(local);
    ...
    {
        severity_color = egui_view_info_bar_mix_disabled(severity_color);
        fill_color = egui_view_info_bar_mix_disabled(fill_color);
        border_color = egui_view_info_bar_mix_disabled(border_color);
        title_color = egui_view_info_bar_mix_disabled(title_color);
        message_color = egui_view_info_bar_mix_disabled(message_color);
        glyph_color = egui_view_info_bar_mix_disabled(glyph_color);
    }
    ...
    egui_view_info_bar_draw_text(local->font, self, title_label, &metrics.title_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, title_color);

    egui_view_info_bar_fit_text_to_width(local->meta_font, snapshot->message, message_label, sizeof(message_label), metrics.message_region.size.width,
                                         local->compact_mode ? 4 : 5);
    egui_view_info_bar_draw_text(local->meta_font, self, message_label, &metrics.message_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, message_color);
...
static uint8_t egui_view_info_bar_hit_part(egui_view_info_bar_t *local, egui_view_t *self, egui_dim_t x, egui_dim_t y)
{
    egui_view_info_bar_metrics_t metrics;

    if (!local->opened)
    {
        return EGUI_VIEW_INFO_BAR_PART_NONE;
    }

    egui_view_info_bar_get_metrics(local, self, &metrics);
    if (metrics.show_action && egui_region_pt_in_rect(&metrics.action_region, x, y))
    {
        return EGUI_VIEW_INFO_BAR_PART_ACTION;
    }
    if (metrics.show_close && egui_region_pt_in_rect(&metrics.close_region, x, y))
    {
        return EGUI_VIEW_INFO_BAR_PART_CLOSE;
    }
...
static int egui_view_info_bar_on_touch_event(egui_view_t *self, egui_motion_event_t *event)
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
    uint8_t hit_part;

    if (!egui_view_info_bar_is_interactive(self, local))
    {
        egui_view_info_bar_clear_pressed_state(self, local);
        return 0;
    }
    ...
    case EGUI_MOTION_EVENT_ACTION_DOWN:
        hit_part = egui_view_info_bar_hit_part(local, self, event->location.x, event->location.y);
        if (hit_part == EGUI_VIEW_INFO_BAR_PART_NONE)
        {
            return 0;
        }
    ...
    case EGUI_MOTION_EVENT_ACTION_MOVE:
        if (local->pressed_part != EGUI_VIEW_INFO_BAR_PART_NONE)
        {
    ...
    case EGUI_MOTION_EVENT_ACTION_UP:
    {
        ...
        hit_part = egui_view_info_bar_hit_part(local, self, event->location.x, event->location.y);
        if (was_pressed && pressed_part != EGUI_VIEW_INFO_BAR_PART_NONE && pressed_part == hit_part)
        {
            if (pressed_part == EGUI_VIEW_INFO_BAR_PART_ACTION)
            {
                egui_view_info_bar_emit_action(self, local);
            }
            else if (pressed_part == EGUI_VIEW_INFO_BAR_PART_CLOSE)
            {
                egui_view_info_bar_set_opened_inner(self, local, 0, 1);
            }
        }
        egui_view_info_bar_clear_pressed_state(self, local);
        return (uint8_t)(hit_part != EGUI_VIEW_INFO_BAR_PART_NONE || pressed_part != EGUI_VIEW_INFO_BAR_PART_NONE);
    }
    case EGUI_MOTION_EVENT_ACTION_CANCEL:
        return egui_view_info_bar_clear_pressed_state(self, local);
    default:
        return 0;
    }
}
...
static int egui_view_info_bar_on_key_event(egui_view_t *self, egui_key_event_t *event)
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
    const egui_view_info_bar_snapshot_t *snapshot;
    ...
    case EGUI_KEY_CODE_ENTER:
    case EGUI_KEY_CODE_SPACE:
        if (egui_view_info_bar_snapshot_has_action(snapshot))
        {
            egui_view_info_bar_emit_action(self, local);
            return 1;
        }
...
static int egui_view_info_bar_on_static_key_event(egui_view_t *self, egui_key_event_t *event)
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
...
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
static int egui_view_info_bar_on_static_touch_event(egui_view_t *self, egui_motion_event_t *event)
{
    EGUI_LOCAL_INIT(egui_view_info_bar_t);
...
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
    api->on_touch_event = egui_view_info_bar_on_static_touch_event;
#endif
#if EGUI_CONFIG_FUNCTION_SUPPORT_KEY
    api->on_key_event = egui_view_info_bar_on_static_key_event;
#endif
}

const egui_view_api_t EGUI_VIEW_API_TABLE_NAME(egui_view_info_bar_t) = {
        .dispatch_touch_event = egui_view_dispatch_touch_event,
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
        .on_touch_event = egui_view_info_bar_on_touch_event,
#else
        ...
        .on_draw = egui_view_info_bar_on_draw,
        .on_detach_from_window = egui_view_on_detach_from_window,
#if EGUI_CONFIG_FUNCTION_SUPPORT_KEY
        .dispatch_key_event = egui_view_dispatch_key_event,
        .on_key_event = egui_view_info_bar_on_key_event,
#endif
};

void egui_view_info_bar_init(egui_view_t *self)
{
    EGUI_INIT_LOCAL(egui_view_info_bar_t);

    egui_view_init(self, uicode_get_core());
    self->api = &EGUI_VIEW_API_TABLE_NAME(egui_view_info_bar_t);
    ...
    local->opened = 0;
    local->pressed_part = EGUI_VIEW_INFO_BAR_PART_NONE;
    egui_view_set_view_name(self, "egui_view_info_bar");
}
