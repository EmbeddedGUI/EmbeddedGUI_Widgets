...
static uint8_t egui_view_dialog_sheet_is_space_char(char c)
{
    return (uint8_t)(c == ' ' || c == '\t');
}

static uint8_t egui_view_dialog_sheet_is_break_after_char(char c)
{
    return (uint8_t)(c == '-' || c == '/');
}

static uint8_t egui_view_dialog_sheet_find_elide_boundary(const char *text, uint8_t visible_chars)
{
    uint8_t index;
    ...
    {
        if (egui_view_dialog_sheet_is_space_char(text[index - 1]))
        {
    ...
    for (index = visible_chars; index > 0; --index)
    {
        if (egui_view_dialog_sheet_is_break_after_char(text[index - 1]))
        {
            return index;
        }
    }
...
static void egui_view_dialog_sheet_copy_elided(char *buffer, uint8_t buffer_size, const char *text, uint8_t max_chars)
{
    ...
    copy_length = egui_view_dialog_sheet_find_elide_boundary(text, (uint8_t)(max_chars - 3));
