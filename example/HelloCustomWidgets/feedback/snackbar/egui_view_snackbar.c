...
static void egui_view_snackbar_on_draw(egui_view_t *self)
{
    ...
    egui_canvas_draw_line(&uicode_get_core()->canvas, region.location.x + 8, region.location.y + region.size.height - 1, region.location.x + region.size.width - 8,
                          region.location.y + region.size.height - 1, 1, egui_rgb_mix(border_color, EGUI_COLOR_WHITE, EGUI_ALPHA_MAKE(20)),
    ...
    if (metrics.show_action)
    {
        action_fill = egui_rgb_mix(local->surface_color, local->accent_color,
                                   EGUI_ALPHA_MAKE(self->is_pressed && local->pressed_part == EGUI_VIEW_SNACKBAR_PART_ACTION ? 16 : 8));
        action_border = egui_rgb_mix(local->border_color, local->accent_color,
                                     EGUI_ALPHA_MAKE(self->is_pressed && local->pressed_part == EGUI_VIEW_SNACKBAR_PART_ACTION ? 30 : 22));
