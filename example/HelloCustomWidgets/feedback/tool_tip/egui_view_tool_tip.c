...
static uint8_t egui_view_tool_tip_is_space_char(char c)
{
    return (uint8_t)(c == ' ' || c == '\t');
}

static uint8_t egui_view_tool_tip_is_break_after_char(char c)
{
    return (uint8_t)(c == '-' || c == '/');
}

static uint8_t egui_view_tool_tip_find_elide_boundary(const char *text, uint8_t visible_chars)
{
    uint8_t index;
    ...
    {
        if (egui_view_tool_tip_is_space_char(text[index - 1]))
        {
    ...
    for (index = visible_chars; index > 0; --index)
    {
        if (egui_view_tool_tip_is_break_after_char(text[index - 1]))
        {
            return index;
        }
    }
...
static void egui_view_tool_tip_copy_elided(char *buffer, uint8_t buffer_size, const char *text, uint8_t max_chars)
{
    ...
    copy_length = egui_view_tool_tip_find_elide_boundary(text, (uint8_t)(max_chars - 3));
...
static egui_dim_t egui_view_tool_tip_measure_text_width(const egui_font_t *font, const char *text)
{
...
{
    static const char *badge_text = "Read only";
    egui_dim_t min_width = local->compact_mode ? 24 : 30;
    egui_dim_t width = (local->compact_mode ? 10 : 12) + egui_view_tool_tip_measure_text_width(local->meta_font, badge_text);
...
static void egui_view_tool_tip_stop_timer(egui_view_t *self)
{
    ...
    egui_timer_stop_timer(uicode_get_core(), &local->show_timer);
...
static void egui_view_tool_tip_start_timer(egui_view_t *self)
{
    ...
    egui_timer_start_timer(uicode_get_core(), &local->show_timer, local->show_delay_ms, 0);
                                           ...
                                           egui_view_tool_tip_metrics_t *metrics)
{
    ...
    target_w = (egui_dim_t)(min_target_w + egui_view_tool_tip_measure_text_width(local->font, snapshot != NULL ? snapshot->target_label : ""));
                                           ...
                                           egui_color_t muted_text_color, egui_color_t border_color, egui_color_t shadow_color)
{
    ...
    else if (local->compact_mode && egui_view_tool_tip_has_text(snapshot->body))
    {
        ...
        egui_dim_t badge_reserved_w = local->read_only_mode ? (egui_view_tool_tip_read_only_badge_width(local) + 4) : 0;
    ...
    if (local->read_only_mode)
    {
        ...
        if (badge_w > metrics->bubble_region.size.width - (local->compact_mode ? 16 : 20))
        {
            badge_w = metrics->bubble_region.size.width - (local->compact_mode ? 16 : 20);
        }
