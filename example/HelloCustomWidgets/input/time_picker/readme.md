...
## 11. 截图复核口径
- 检查目录：`runtime_check_output/HelloCustomWidgets_input_time_picker/default`
- 本轮复核结果：
  - 共捕获 `11` 帧
  - 全帧共出现 `4` 组唯一状态，主区哈希分组为 `[0,1,8,9,10] / [2,3] / [4,5] / [6,7]`
  - 主区 RGB 差分边界收敛到 `(22, 82) - (457, 391)`
  - 遮罩主区变化边界后，主区外区域唯一哈希数为 `1`
  - 按 `y >= 392` 裁切底部 preview 后，preview 区唯一哈希数为 `1`
...
- 本次保留核心状态：
  ...
  - `read only`
- 删减的装饰或桥接：
  - 页面级 guide、状态文案、`Standard` 标签、分隔线和 preview label
  - 标签点击切换和额外页面层状态桥接
  - 桌面级浮层定位、Acrylic、复杂阴影和过渡动画
  - 场景化说明卡片和装饰性 chrome
...
- `HelloUnitTest`：`日志复核 PASS`
  - `make all APP=HelloUnitTest PORT=pc_test`
  - `X:\output\main.exe`
  - 当前环境下直接执行超时且重定向日志为空，本轮按本地 unit 日志复核总计 `845 / 845`，其中 `time_picker` suite `8 / 8`
