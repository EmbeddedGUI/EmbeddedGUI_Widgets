...
#define COMMAND_BAR_FLYOUT_RECORD_FINAL_WAIT 280
#define COMMAND_BAR_FLYOUT_DEFAULT_SNAPSHOT  0
...
static const egui_view_command_bar_flyout_snapshot_t compact_snapshot = {
...
{
    egui_view_command_bar_flyout_set_current_snapshot(EGUI_VIEW_OF(&flyout_primary), index % PRIMARY_SNAPSHOT_COUNT);
    if (ui_ready)
    {
        layout_page();
    }
...
{
    apply_primary_snapshot(COMMAND_BAR_FLYOUT_DEFAULT_SNAPSHOT);
}

static void apply_preview_states(void)
{
    egui_view_command_bar_flyout_set_snapshots(EGUI_VIEW_OF(&flyout_compact), &compact_snapshot, 1);
    egui_view_command_bar_flyout_set_current_snapshot(EGUI_VIEW_OF(&flyout_compact), 0);
    egui_view_command_bar_flyout_set_snapshots(EGUI_VIEW_OF(&flyout_disabled), &disabled_snapshot, 1);
    egui_view_command_bar_flyout_set_current_snapshot(EGUI_VIEW_OF(&flyout_disabled), 0);
    if (ui_ready)
    {
        layout_page();
    }
...
static void focus_primary_flyout(void)
{
...
static void apply_primary_key(uint8_t key_code)
{
    ...
    focus_primary_flyout();
    event.type = EGUI_KEY_EVENT_ACTION_DOWN;
    event.key_code = key_code;
    egui_view_dispatch_key_event(EGUI_VIEW_OF(&flyout_primary), &event);
...
void test_init_ui(void)
{
...
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
    egui_view_set_focusable(EGUI_VIEW_OF(&flyout_primary), 1);
#endif
    ...
    egui_view_command_bar_flyout_set_font(EGUI_VIEW_OF(&flyout_compact), (const egui_font_t *)&egui_res_font_montserrat_10_4);
    ...
    egui_view_command_bar_flyout_set_font(EGUI_VIEW_OF(&flyout_disabled), (const egui_font_t *)&egui_res_font_montserrat_10_4);
    ...
    egui_view_command_bar_flyout_set_palette(EGUI_VIEW_OF(&flyout_disabled), HCW_COLOR_PANEL, HCW_COLOR_PANEL,
                                             HCW_COLOR_BORDER_STRONG, HCW_COLOR_TEXT, HCW_COLOR_TEXT_SOFT,
                                             HCW_COLOR_TEXT_SOFT, HCW_COLOR_TEXT_SOFT, HCW_COLOR_WARNING_DARK,
                                             HCW_COLOR_DANGER_DARK, HCW_COLOR_BORDER_STRONG, HCW_COLOR_BORDER_STRONG);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 6:
        ...
        return true;
    case 7:
        EGUI_SIM_SET_WAIT(p_action, COMMAND_BAR_FLYOUT_RECORD_FINAL_WAIT);
        return true;
    case 8:
        if (first_call)
        {
