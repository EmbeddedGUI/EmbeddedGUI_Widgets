...
| 录制最终稳定帧 | 空文本，仅显示 placeholder | 保持不变 | 保持不变 |
| 静态 preview 对照 | 否 | 是 | 是 |
...
- 底部 preview 统一通过 `hcw_text_box_override_static_preview_api()` 吞掉 `touch / key`
...
- `HelloUnitTest`：`845 / 845` 通过，其中 `text_box` suite `6 / 6`
...
- wasm 构建：PASS（`web/demos/HelloCustomWidgets_input_text_box`）
- web smoke：`PASS status=Running canvas=480x480 ratio=0.0977 colors=103`
