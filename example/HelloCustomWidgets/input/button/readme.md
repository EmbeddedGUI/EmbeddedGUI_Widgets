...
- 参考开源库：`WPF UI`
- 对应组件：`Button`
- 当前保留形态：`Deploy`、`Sync`、`Confirm`、`Dispatch`、`Secondary`、`Disabled`
- 当前保留交互：主区保留真实 `touch` 与 `Space / Enter` 提交；底部 `secondary / disabled` preview 统一收口为静态 reference 对照
- 当前移除内容：控件 helper 层的独立紧凑样式 API、页面级 guide、状态说明文案、preview 清焦桥接、录制阶段 preview 快照轮换、额外 showcase 化装饰，以及旧版 finalize README 章节结构
- EGUI 适配说明：目录和 demo 继续使用 `input/button`，底层仍复用仓库内现有 `hcw_button` 实现；小尺寸预览由 APP 侧尺寸、字体和 gap 配置，不修改 `sdk/EmbeddedGUI`
...
- 左侧 preview：`secondary`，固定显示 `Open`
...
- 单个 preview：`96 x 32`
- 页面结构：标题 -> 主 `button` -> 底部 `secondary / disabled`
- 风格约束：主按钮使用 Fluent 风格蓝色主动作视觉；`secondary` preview 只由 APP 侧尺寸、字体与间距配置缩小；`disabled` preview 使用浅灰低对比视觉；焦点态只保留轻量 ring，不引入厚阴影或额外帮助文案

## 6. 状态矩阵
| 状态 | 主按钮 | Secondary preview | Disabled preview |
| --- | --- | --- | --- |
...
1. 样式 helper 更新背景并清理 `pressed`。
   覆盖 `apply_standard_style()` 之后的基线，以及 `apply_disabled_style()` 的背景、文字色和 `icon_text_gap` 更新。
...
- 底部 `secondary / disabled` preview 统一通过 `hcw_button_override_static_preview_api()` 吞掉 `touch / key`，只承担静态 reference 对照，不触发 click。

## 8. 录制动作设计
`egui_port_get_recording_action()` 保留真实主按钮交互，但底部 preview 已收口为静态 reference 工作流：

1. 恢复主按钮默认 `Deploy`，同时恢复底部 `secondary / disabled` 固定状态并抓取首帧，等待 `BUTTON_RECORD_FRAME_WAIT`。
...
## 9. 验收命令
```bash
make all APP=HelloCustomWidgets APP_SUB=input/button PORT=pc COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0
...
python scripts/code_runtime_check.py --app HelloCustomWidgets --app-sub input/button --timeout 10 --keep-screenshots
make all APP=HelloCustomWidgets APP_SUB=input/button PORT=emscripten COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0
git diff --check
```

## 10. 验收重点
- 主按钮和底部 `secondary / disabled` preview 必须完整可见，不能黑屏、白屏或裁切。
...
- `HelloUnitTest`：`PASS`
  - `make all APP=HelloUnitTest PORT=pc_test COMPILE_DEBUG= COMPILE_OPT_LEVEL=-O0`
  - `.\output\main.exe`
  - 全量结果：`1048 / 1048`，其中 `button` suite `7 / 7`
...
- 截图复核结论：主区应覆盖默认 `Deploy`、`Sync`、`Confirm` 与 `Dispatch` 四组 reference 状态，最终稳定帧保持 `Dispatch`，底部 `secondary / disabled` preview 全程静态
