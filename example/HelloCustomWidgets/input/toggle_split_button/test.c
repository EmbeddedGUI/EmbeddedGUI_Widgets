...
{
    egui_view_toggle_split_button_set_current_snapshot(EGUI_VIEW_OF(&button_primary), index % EGUI_ARRAY_SIZE(primary_snapshots));
}

static void apply_compact_snapshot(uint8_t index)
{
    egui_view_toggle_split_button_set_current_snapshot(EGUI_VIEW_OF(&button_compact), index % EGUI_ARRAY_SIZE(compact_snapshots));
}
...
void test_init_ui(void)
{
    ...
    egui_view_toggle_split_button_set_font(EGUI_VIEW_OF(&button_read_only), (const egui_font_t *)&egui_res_font_montserrat_10_4);
