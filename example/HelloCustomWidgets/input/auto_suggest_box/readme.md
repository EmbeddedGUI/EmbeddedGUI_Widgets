...
- 参考开源库：`WPF UI`
- 对应组件：`AutoSuggestBox`
- 当前保留形态：`Ali`、people suggestions、`Alicia Gomez`、`Deploy Worker`、`compact`、`read only`
...
2. people suggestions expanded
   默认建议列表展开
3. `Alicia Gomez` highlighted
   键盘向下导航后的展开态
4. `Deploy Worker` committed
   切到命令集后 `Down + End + Enter` 提交结果
...
| --- | --- | --- | --- |
| 默认显示 | `Ali` | `Recent` | `Recent` |
| 快照 2 | people suggestions expanded | 保持不变 | 保持不变 |
| 快照 3 | `Alicia Gomez` highlighted | 保持不变 | 保持不变 |
...
1. suggestions、query、current index 与 current text 基线。
   覆盖 `set_suggestions()`、`set_query()`、`set_current_index()` 与 query 过滤后的索引回退。
2. 样式 helper 与 params。
   覆盖 `apply_standard_style()`、`apply_compact_style()`、`apply_read_only_style()`、`set_font()`、`set_icon_font()`、`set_max_visible_items()`，以及 `apply_params()` / `init_with_params()`。
...
5. 键盘导航与提交。
   覆盖可打印字符输入、`Down / End / Enter / Escape` 的过滤、导航、提交与收起闭环。
6. `disabled / empty` guard。
   覆盖 `!enable` 拒绝输入，以及空 suggestions 下字段仍可获得触摸焦点但不会展开建议列表。
7. static preview 吞掉输入且保持状态不变。
   固定校验 `suggestions / suggestion_count / current_index / query / collapsed_height / item_height / max_visible_items / colors / font / region_screen` 不变，并要求 `pressed_part / pressed_row / is_pressed` 被清理且 `on_selected` 不触发。

说明：
- 主区真实交互继续保留建议展开、键盘导航和提交；录制路径主要走 `Down / End / Enter` 的键盘闭环与显式命令快照切换。
...
1. 恢复主控件默认 `Ali`，同时恢复底部 `compact / read only` 固定状态并抓取首帧，等待 `AUTO_SUGGEST_BOX_RECORD_FRAME_WAIT`。
2. 发送一次 `Down`，展开 people suggestions，等待 `AUTO_SUGGEST_BOX_RECORD_WAIT`。
3. 抓取第二组主区快照，等待 `AUTO_SUGGEST_BOX_RECORD_FRAME_WAIT`。
4. 再发送一次 `Down`，把高亮移动到 `Alicia Gomez`，等待 `AUTO_SUGGEST_BOX_RECORD_WAIT`。
5. 抓取第三组主区快照，等待 `AUTO_SUGGEST_BOX_RECORD_FRAME_WAIT`。
6. 切换到命令 suggestions，并发送 `Down + End + Enter`，提交 `Deploy Worker`，等待 `AUTO_SUGGEST_BOX_RECORD_WAIT`。
...
- 本轮复核结果：
  - 共捕获 `10` 帧
  - 可以稳定区分 `默认输入 / suggestions 展开 / 高亮导航 / 命令提交` 四组主区状态
...
- 保留的交互：
  - 建议展开
  - 键盘输入过滤
  - 键盘 `Down / Up / Home / End / Enter / Escape`
  - 命令提交
  - 静态 preview 对照
- 删减的旧桥接与旧装饰：
  - 页面级 guide 与状态文案
...
- 触摸语义：`PASS`
  - `python scripts/checks/check_touch_release_semantics.py --scope custom --category input`
  - 结果：`custom_audited=28 custom_skipped_allowlist=5`
- 单控件 runtime：`PASS`
  ...
  - 共捕获 `10` 帧
- 截图复核结论：
  - 主区覆盖默认 `Ali`、展开态、高亮 `Alicia Gomez` 与 `Deploy Worker` 提交结果四组 reference 状态
  - 最终稳定帧保持 `Deploy Worker`
  - 底部 `compact / read only` preview 在整条 reference 轨道中保持静态不变
