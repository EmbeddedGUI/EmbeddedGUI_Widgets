...
#define SEARCH_BOX_RECORD_FINAL_WAIT 520
...
{
    egui_view_search_box_set_placeholder(EGUI_VIEW_OF(&box_primary), "Search templates");
    egui_view_search_box_set_text(EGUI_VIEW_OF(&box_primary), "Roadmap");
    egui_view_search_box_set_max_length(EGUI_VIEW_OF(&box_primary), 24);
}

static void apply_compact_state(const char *text)
{
    egui_view_search_box_set_placeholder(EGUI_VIEW_OF(&box_compact), "Recent");
    egui_view_search_box_set_text(EGUI_VIEW_OF(&box_compact), text);
    egui_view_search_box_set_max_length(EGUI_VIEW_OF(&box_compact), 16);
}

static void apply_read_only_state(void)
{
    egui_view_search_box_set_placeholder(EGUI_VIEW_OF(&box_read_only), "Pinned");
    egui_view_search_box_set_text(EGUI_VIEW_OF(&box_read_only), "Assets");
    egui_view_search_box_set_max_length(EGUI_VIEW_OF(&box_read_only), 16);
}
...
static void dismiss_primary_search_box(void)
{
...
void test_init_ui(void)
{
    ...
    egui_view_search_box_override_static_preview_api(EGUI_VIEW_OF(&box_compact), &compact_api);
    ...
    egui_view_search_box_override_static_preview_api(EGUI_VIEW_OF(&box_read_only), &read_only_api);
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
...
static void set_click_clear_action(egui_sim_action_t *p_action, egui_view_t *view, int interval_ms)
{
    egui_region_t region;

    if (!egui_view_search_box_get_clear_region(view, &region))
    {
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 2:
        if (first_call)
        {
            apply_primary_key(EGUI_KEY_CODE_S, 0);
            recording_request_snapshot();
        }
        EGUI_SIM_SET_WAIT(p_action, SEARCH_BOX_RECORD_FRAME_WAIT);
        return true;
    case 3:
        ...
        return true;
    case 4:
        set_click_clear_action(p_action, EGUI_VIEW_OF(&box_primary), SEARCH_BOX_RECORD_WAIT);
        return true;
    case 5:
        if (first_call)
        {
        ...
        return true;
    case 6:
        EGUI_SIM_SET_CLICK_VIEW(p_action, &box_compact, SEARCH_BOX_RECORD_WAIT);
        return true;
    case 7:
        if (first_call)
        {
