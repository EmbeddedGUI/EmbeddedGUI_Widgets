...
#define GRID_VIEW_RECORD_FINAL_WAIT 520
...
static const hcw_grid_view_item_t compact_items_1[] = {
        {"Q", "Crew", "4", HCW_GRID_VIEW_TONE_NEUTRAL, 0, HCW_GRID_VIEW_WIDTH_COMPACT},
        {"R", "Tiles", "2", HCW_GRID_VIEW_TONE_ACCENT, 1, HCW_GRID_VIEW_WIDTH_BALANCED},
        {"S", "Views", "6", HCW_GRID_VIEW_TONE_SUCCESS, 0, HCW_GRID_VIEW_WIDTH_COMPACT},
        {"T", "Notes", "7", HCW_GRID_VIEW_TONE_WARNING, 0, HCW_GRID_VIEW_WIDTH_BALANCED},
};

static const hcw_grid_view_snapshot_t compact_snapshots[] = {
        {"AST", "Compact grid", "", "", compact_items_0, 4, 0, HCW_GRID_VIEW_LAYOUT_WRAP},
        {"TEAM", "Compact board", "", "", compact_items_1, 4, 1, HCW_GRID_VIEW_LAYOUT_WRAP},
};
...
{
    hcw_grid_view_set_current_snapshot(EGUI_VIEW_OF(&grid_view_compact), index % COMPACT_SNAPSHOT_COUNT);
}
...
static int dismiss_primary_focus_on_preview_touch(egui_view_t *self, egui_motion_event_t *event)
{
    ...
    {
        egui_view_clear_focus(EGUI_VIEW_OF(&grid_view_primary));
    }
#else
    EGUI_UNUSED(event);
#endif
...
static void apply_primary_key(uint8_t key_code)
{
    ...
    EGUI_VIEW_OF(&grid_view_primary)->api->dispatch_key_event(EGUI_VIEW_OF(&grid_view_primary), &event);
...
static void set_click_item(egui_sim_action_t *p_action, egui_view_t *view, uint8_t item_index, int interval_ms)
{
    ...
    if (!hcw_grid_view_get_item_region(view, item_index, &region))
    {
...
void test_init_ui(void)
{
    ...
    hcw_grid_view_set_snapshots(EGUI_VIEW_OF(&grid_view_compact), compact_snapshots, COMPACT_SNAPSHOT_COUNT);
    ...
    grid_view_compact_api.on_touch = dismiss_primary_focus_on_preview_touch;
#endif
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
...
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
    grid_view_read_only_api.on_touch = dismiss_primary_focus_on_preview_touch;
#endif
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 10:
        if (first_call)
        {
            set_click_item(p_action, EGUI_VIEW_OF(&grid_view_primary), 0, 220);
        }
        return true;
    case 11:
        if (first_call)
    ...
    case 19:
        ...
        return true;
    case 20:
        set_click_view_center(p_action, EGUI_VIEW_OF(&grid_view_compact), 220);
        return true;
    case 21:
        if (first_call)
        {
