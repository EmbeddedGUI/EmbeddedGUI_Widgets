...
录制只导出主控件的状态变化。底部两个 preview 在整条 reference 轨道中保持静态一致，不再承担 preview dismiss 或焦点清理职责。当前 `test.c` 已把恢复默认态后的等待与最终抓帧统一收口到 `SCROLL_PRESENTER_RECORD_FINAL_WAIT`，中间状态切换继续保留 `SCROLL_PRESENTER_RECORD_WAIT / SCROLL_PRESENTER_RECORD_FRAME_WAIT`。
...
- 截图复核结论：
  ...
  - 按 `y >= 294` 裁切底部 preview 后保持单一哈希，确认 `compact / read only` preview 全程静态
