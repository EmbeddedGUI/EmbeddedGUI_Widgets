...
static void wrap_panel_copy_elided(char *buffer, uint8_t buffer_size, const char *text, uint8_t max_chars)
{
    uint8_t copy_length;
    uint8_t index;
    uint8_t length;
    ...
    length = wrap_panel_text_len(text);
    if (length <= max_chars)
    {
        copy_length = length;
...
static egui_dim_t wrap_panel_measure_font_line_height(const egui_font_t *font)
{
...
static egui_dim_t wrap_panel_measure_text_width(const egui_font_t *font, const char *text)
{
    egui_dim_t text_width = 0;
    egui_dim_t dummy_height = 0;

    if (!wrap_panel_has_text(text) || font == NULL || font->api == NULL || font->api->get_str_size == NULL)
    {
        return 0;
    }
...
static void wrap_panel_fit_text_to_width(const egui_font_t *font, const char *text, char *buffer, uint8_t buffer_size, egui_dim_t max_width,
                                         egui_dim_t fallback_char_width)
{
    ...
    buffer[0] = '\0';
    if (!wrap_panel_has_text(text) || max_width <= 0)
    {
        return;
    }

    max_chars = wrap_panel_text_len(text);
    wrap_panel_copy_elided(buffer, buffer_size, text, max_chars);
    while (max_chars > 0)
    {
        egui_dim_t text_width = wrap_panel_measure_text_width(font, buffer);

        if (text_width <= 0)
        {
            text_width = (egui_dim_t)wrap_panel_text_len(buffer) * fallback_char_width;
        }
...
static egui_dim_t wrap_panel_get_title_height(egui_view_wrap_panel_t *local)
{
    egui_dim_t title_h = wrap_panel_measure_font_line_height(local->font);
    egui_dim_t min_h = local->compact_mode ? EGUI_VIEW_WRAP_PANEL_COMPACT_TITLE_H : EGUI_VIEW_WRAP_PANEL_STANDARD_TITLE_H;

    return title_h > min_h ? title_h : min_h;
}

static egui_dim_t wrap_panel_get_meta_height(egui_view_wrap_panel_t *local)
{
    egui_dim_t meta_h = wrap_panel_measure_font_line_height(local->meta_font);
    egui_dim_t min_h = local->compact_mode ? EGUI_VIEW_WRAP_PANEL_COMPACT_BADGE_H : EGUI_VIEW_WRAP_PANEL_STANDARD_BADGE_H;

    return meta_h > min_h ? meta_h : min_h;
}

static egui_dim_t wrap_panel_get_summary_height(egui_view_wrap_panel_t *local)
{
    if (local->compact_mode)
    {
        return 0;
    }
    return wrap_panel_get_meta_height(local);
}

static egui_dim_t wrap_panel_get_footer_height(egui_view_wrap_panel_t *local)
{
    if (local->compact_mode)
    {
        return 0;
    ...
    {
        egui_dim_t footer_h = wrap_panel_measure_font_line_height(local->meta_font);

        return footer_h > EGUI_VIEW_WRAP_PANEL_STANDARD_FOOTER_H ? footer_h : EGUI_VIEW_WRAP_PANEL_STANDARD_FOOTER_H;
    }
}

static egui_dim_t wrap_panel_get_item_height(egui_view_wrap_panel_t *local)
{
    egui_dim_t content_h = wrap_panel_get_meta_height(local);
    egui_dim_t title_h = wrap_panel_get_title_height(local);
    egui_dim_t min_h = local->compact_mode ? EGUI_VIEW_WRAP_PANEL_COMPACT_ITEM_H : EGUI_VIEW_WRAP_PANEL_STANDARD_ITEM_H;
    egui_dim_t padded_h;
...
static egui_dim_t wrap_panel_pill_width(const egui_font_t *font, const char *text, uint8_t compact_mode, egui_dim_t min_width, egui_dim_t max_width)
{
    egui_dim_t width = wrap_panel_measure_text_width(font, text) + (compact_mode ? 10 : 12);
...
static egui_dim_t wrap_panel_item_width(egui_view_wrap_panel_t *local, const egui_view_wrap_panel_item_t *item, egui_dim_t max_width)
{
    ...
    {
        width += wrap_panel_pill_width(local->meta_font, item->badge, local->compact_mode, local->compact_mode ? 12 : 16, max_width);
        has_content = 1;
    }
    if (wrap_panel_has_text(item->title))
    {
        ...
        width += wrap_panel_measure_text_width(local->font, item->title);
        has_content = 1;
    }
    if (wrap_panel_has_text(item->meta))
    {
        ...
        width += wrap_panel_measure_text_width(local->meta_font, item->meta) + (local->compact_mode ? 4 : 6);
    }
                                   ...
                                   egui_view_wrap_panel_metrics_t *metrics)
{
    ...
    egui_dim_t footer_h = wrap_panel_get_footer_height(local);
    ...
    egui_dim_t item_h = wrap_panel_get_item_height(local);
    egui_dim_t inner_x;
    egui_dim_t inner_y;
    egui_dim_t inner_w;
    egui_dim_t inner_h;
    ...
    if (wrap_panel_has_text(snapshot->header))
    {
        ...
        metrics->badge_region.size.width = wrap_panel_pill_width(local->meta_font, snapshot->header, local->compact_mode, local->compact_mode ? 18 : 24, inner_w);
    ...
    if (footer_h > 0 && wrap_panel_has_text(snapshot->footer))
    {
        ...
        metrics->footer_region.size.width = wrap_panel_pill_width(local->meta_font, snapshot->footer, local->compact_mode, local->compact_mode ? 18 : 22, inner_w);
                                 ...
                                 uint8_t item_index)
{
    ...
    egui_color_t fill_color =
            egui_rgb_mix(local->surface_color, tone_color,
                         EGUI_ALPHA_MAKE(self->is_pressed && local->pressed_item == item_index ? 12 : (item_index == local->current_item ? 8 : (item->emphasized ? 6 : 3))));
    egui_color_t border_color =
            egui_rgb_mix(local->border_color, tone_color,
                         EGUI_ALPHA_MAKE(self->is_pressed && local->pressed_item == item_index ? 28 : (item_index == local->current_item ? 22 : 16)));
    egui_color_t badge_fill = egui_rgb_mix(local->section_color, tone_color, EGUI_ALPHA_MAKE(item_index == local->current_item ? 14 : 8));
    egui_color_t badge_text = egui_rgb_mix(local->text_color, tone_color, EGUI_ALPHA_MAKE(12));
    egui_color_t title_color = egui_rgb_mix(local->text_color, tone_color, EGUI_ALPHA_MAKE(item_index == local->current_item ? 10 : 6));
    egui_color_t meta_color = egui_rgb_mix(local->muted_text_color, tone_color, EGUI_ALPHA_MAKE(item->emphasized ? 12 : 8));
    ...
    egui_dim_t badge_h = wrap_panel_get_meta_height(local);
    egui_dim_t title_h = wrap_panel_get_title_height(local);
    egui_dim_t meta_h = wrap_panel_get_meta_height(local);
    egui_dim_t gap = local->compact_mode ? 4 : 5;
    egui_region_t badge_region;
    egui_region_t title_region;
    ...
    if (wrap_panel_has_text(item->badge))
    {
        badge_w = wrap_panel_pill_width(local->meta_font, item->badge, local->compact_mode, local->compact_mode ? 12 : 16, region->size.width / 2);
        ...
        wrap_panel_fit_text_to_width(local->meta_font, item->badge, badge_label, sizeof(badge_label), badge_region.size.width - 4, 4);
        wrap_panel_draw_text(local->meta_font, self, badge_label, &badge_region, EGUI_ALIGN_CENTER, badge_text);
    }

    if (wrap_panel_has_text(item->meta))
    {
        meta_w = wrap_panel_measure_text_width(local->meta_font, item->meta) + (local->compact_mode ? 4 : 6);
    ...
    wrap_panel_fit_text_to_width(local->font, item->title, title_label, sizeof(title_label), title_region.size.width, local->compact_mode ? 4 : 5);
    wrap_panel_draw_text(local->font, self, title_label, &title_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, title_color);
    wrap_panel_fit_text_to_width(local->meta_font, item->meta, meta_label, sizeof(meta_label), meta_region.size.width - 2, 4);
    wrap_panel_draw_text(local->meta_font, self, meta_label, &meta_region, EGUI_ALIGN_RIGHT | EGUI_ALIGN_VCENTER, meta_color);
...
static void egui_view_wrap_panel_on_draw(egui_view_t *self)
{
    ...
    egui_color_t badge_fill = egui_rgb_mix(local->section_color, local->accent_color, EGUI_ALPHA_MAKE(10));
    egui_color_t badge_text = egui_rgb_mix(local->text_color, local->accent_color, EGUI_ALPHA_MAKE(12));
    egui_color_t title_color = local->text_color;
    egui_color_t summary_color = local->muted_text_color;
    egui_color_t shell_fill = egui_rgb_mix(local->section_color, HCW_COLOR_SURFACE_SUBTLE, EGUI_ALPHA_MAKE(18));
    egui_color_t shell_border = egui_rgb_mix(local->border_color, local->accent_color, EGUI_ALPHA_MAKE(20));
    egui_color_t footer_fill = egui_rgb_mix(local->surface_color, local->accent_color, EGUI_ALPHA_MAKE(8));
    egui_color_t footer_text = egui_rgb_mix(local->muted_text_color, local->accent_color, EGUI_ALPHA_MAKE(10));
    ...
    egui_canvas_draw_round_rectangle(&uicode_get_core()->canvas, metrics.content_region.location.x, metrics.content_region.location.y, metrics.content_region.size.width,
                                     metrics.content_region.size.height, radius, 1, card_border, egui_color_alpha_mix(self->alpha, EGUI_ALPHA_MAKE(local->read_only_mode ? 76 : 92)));

    if (self->is_focused && egui_view_get_enable(self) && !local->read_only_mode)
    {
        egui_canvas_draw_round_rectangle(&uicode_get_core()->canvas, metrics.content_region.location.x, metrics.content_region.location.y, metrics.content_region.size.width,
                                         metrics.content_region.size.height, radius, 2, focus_color, egui_color_alpha_mix(self->alpha, EGUI_ALPHA_MAKE(94)));
    }
    ...
    if (snapshot != NULL && wrap_panel_region_has_size(&metrics.badge_region))
    {
        egui_canvas_draw_round_rectangle_fill(&uicode_get_core()->canvas, metrics.badge_region.location.x, metrics.badge_region.location.y, metrics.badge_region.size.width,
                                              ...
                                              egui_color_alpha_mix(self->alpha, EGUI_ALPHA_MAKE(98)));
        wrap_panel_fit_text_to_width(local->meta_font, snapshot->header, header_label, sizeof(header_label), metrics.badge_region.size.width - 4, 4);
        wrap_panel_draw_text(local->meta_font, self, header_label, &metrics.badge_region, EGUI_ALIGN_CENTER, badge_text);
    }

    if (snapshot != NULL)
    {
        wrap_panel_fit_text_to_width(local->font, snapshot->title, title_label, sizeof(title_label), metrics.title_region.size.width, local->compact_mode ? 4 : 5);
        wrap_panel_draw_text(local->font, self, title_label, &metrics.title_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, title_color);
        wrap_panel_fit_text_to_width(local->meta_font, snapshot->summary, summary_label, sizeof(summary_label), metrics.summary_region.size.width, 4);
        wrap_panel_draw_text(local->meta_font, self, summary_label, &metrics.summary_region, EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER, summary_color);
    }
    ...
    if (wrap_panel_region_has_size(&metrics.shell_region))
    {
        ...
        egui_canvas_draw_round_rectangle(&uicode_get_core()->canvas, metrics.shell_region.location.x, metrics.shell_region.location.y, metrics.shell_region.size.width,
                                         metrics.shell_region.size.height, shell_radius, 1, shell_border,
    ...
    if (snapshot != NULL && wrap_panel_region_has_size(&metrics.footer_region))
    {
        ...
        wrap_panel_fit_text_to_width(local->meta_font, snapshot->footer, footer_label, sizeof(footer_label), metrics.footer_region.size.width - 4, 4);
        wrap_panel_draw_text(local->meta_font, self, footer_label, &metrics.footer_region, EGUI_ALIGN_CENTER, footer_text);
    }
}
