...
#define ITEMS_CONTROL_ROOT_WIDTH        238
#define ITEMS_CONTROL_ROOT_HEIGHT       232
#define ITEMS_CONTROL_PRIMARY_WIDTH     176
#define ITEMS_CONTROL_PRIMARY_HEIGHT    104
#define ITEMS_CONTROL_ITEM_COUNT        4
#define ITEMS_CONTROL_PREVIEW_ROW_WIDTH 202
#define ITEMS_CONTROL_PREVIEW_WIDTH     92
#define ITEMS_CONTROL_PREVIEW_HEIGHT    54
#define ITEMS_CONTROL_RECORD_WAIT       90
#define ITEMS_CONTROL_RECORD_FRAME_WAIT 170
#define ITEMS_CONTROL_RECORD_FINAL_WAIT 280
#define ITEMS_CONTROL_DEFAULT_SNAPSHOT  0

#define PRIMARY_SNAPSHOT_COUNT ((uint8_t)EGUI_ARRAY_SIZE(primary_snapshots))

typedef struct items_control_snapshot items_control_snapshot_t;
struct items_control_snapshot
{
    const char *items[ITEMS_CONTROL_ITEM_COUNT];
    const char *caption;
...
static egui_view_items_control_t primary_control;
static egui_view_label_t primary_items[ITEMS_CONTROL_ITEM_COUNT];
static egui_view_label_t caption_label;
static egui_view_linearlayout_t bottom_row;
static egui_view_items_control_t wrap_preview;
static egui_view_label_t wrap_items[3];
static egui_view_items_control_t read_only_preview;
...
static const char *title_text = "ItemsControl";

static const items_control_snapshot_t primary_snapshots[] = {
        {
...
static void init_item_label(egui_view_label_t *label, egui_dim_t width, egui_dim_t height, const char *text, const egui_font_t *font,
                            egui_color_t color)
{
...
static void apply_items_control_style(egui_view_t *view, uint8_t style)
{
    switch (style)
    {
    case 1:
        egui_view_items_control_apply_strip_style(view);
        break;
    case 2:
        egui_view_items_control_apply_wrap_style(view);
        break;
    case 3:
        egui_view_items_control_apply_read_only_style(view);
        break;
    default:
        egui_view_items_control_apply_standard_style(view);
        break;
    }
}
...
{
    uint8_t i;
    const items_control_snapshot_t *snapshot = &primary_snapshots[index % PRIMARY_SNAPSHOT_COUNT];

    apply_items_control_style(EGUI_VIEW_OF(&primary_control), snapshot->style);
    ...
    egui_view_items_control_layout_items(EGUI_VIEW_OF(&primary_control));
    if (ui_ready)
    {
        layout_page();
    }
...
{
    apply_primary_snapshot(ITEMS_CONTROL_DEFAULT_SNAPSHOT);
}

static void apply_preview_states(void)
{
    egui_view_items_control_apply_wrap_style(EGUI_VIEW_OF(&wrap_preview));
    egui_view_label_set_text(EGUI_VIEW_OF(&wrap_items[0]), "Wrap");
    egui_view_label_set_text(EGUI_VIEW_OF(&wrap_items[1]), "Chip");
    egui_view_label_set_text(EGUI_VIEW_OF(&wrap_items[2]), "Host");
    egui_view_items_control_layout_items(EGUI_VIEW_OF(&wrap_preview));

    egui_view_items_control_apply_read_only_style(EGUI_VIEW_OF(&read_only_preview));
    egui_view_label_set_text(EGUI_VIEW_OF(&read_only_items[0]), "Read");
    egui_view_label_set_text(EGUI_VIEW_OF(&read_only_items[1]), "Only");
    egui_view_label_set_text(EGUI_VIEW_OF(&read_only_items[2]), "List");
    egui_view_items_control_layout_items(EGUI_VIEW_OF(&read_only_preview));
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), ITEMS_CONTROL_ROOT_WIDTH, ITEMS_CONTROL_ROOT_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&root_layout), 0);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&root_layout), EGUI_ALIGN_HCENTER);
    egui_view_set_background(EGUI_VIEW_OF(&root_layout), EGUI_BG_OF(&bg_page_panel));

    egui_view_label_init(EGUI_VIEW_OF(&title_label), uicode_get_core());
    egui_view_set_size(EGUI_VIEW_OF(&title_label), ITEMS_CONTROL_ROOT_WIDTH, 18);
    ...
    egui_view_items_control_init(EGUI_VIEW_OF(&primary_control));
    egui_view_set_size(EGUI_VIEW_OF(&primary_control), ITEMS_CONTROL_PRIMARY_WIDTH, ITEMS_CONTROL_PRIMARY_HEIGHT);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&caption_label), ITEMS_CONTROL_ROOT_WIDTH, 12);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), ITEMS_CONTROL_PREVIEW_ROW_WIDTH, ITEMS_CONTROL_PREVIEW_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&bottom_row), 1);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&bottom_row), EGUI_ALIGN_VCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&bottom_row));

    egui_view_items_control_init(EGUI_VIEW_OF(&wrap_preview));
    ...
    egui_view_items_control_init(EGUI_VIEW_OF(&read_only_preview));
    egui_view_set_size(EGUI_VIEW_OF(&read_only_preview), ITEMS_CONTROL_PREVIEW_WIDTH, ITEMS_CONTROL_PREVIEW_HEIGHT);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        EGUI_SIM_SET_WAIT(p_action, ITEMS_CONTROL_RECORD_FRAME_WAIT);
        return true;
    case 1:
        if (first_call)
        {
        ...
        EGUI_SIM_SET_WAIT(p_action, ITEMS_CONTROL_RECORD_WAIT);
        return true;
    case 2:
        if (first_call)
        {
    ...
    case 8:
        ...
        EGUI_SIM_SET_WAIT(p_action, ITEMS_CONTROL_RECORD_FINAL_WAIT);
        return true;
    default:
        return false;
    }
