...
#define GRID_PRIMARY_GRID_HEIGHT    62
...
#define GRID_CARD_TEXT_PAD_X        6
#define GRID_CARD_COMPACT_PAD_X     4
#define GRID_CARD_BODY_PAD_Y        3
#define GRID_CARD_COMPACT_PAD_Y     1
...
static egui_view_api_t primary_eyebrow_apis[GRID_CARD_CAPACITY];
static egui_view_api_t primary_title_apis[GRID_CARD_CAPACITY];
static egui_view_api_t stack_eyebrow_apis[GRID_STACK_PREVIEW_CARDS];
static egui_view_api_t stack_title_apis[GRID_STACK_PREVIEW_CARDS];
static egui_view_api_t dense_eyebrow_apis[GRID_DENSE_PREVIEW_CARDS];
static egui_view_api_t dense_title_apis[GRID_DENSE_PREVIEW_CARDS];
...
static void grid_centered_label_on_draw(egui_view_t *self)
{
...
static void grid_apply_centered_label_api(egui_view_t *label, egui_view_api_t *api)
{
    egui_view_copy_api(label, api);
    api->on_draw = grid_centered_label_on_draw;
}
                      ...
                      egui_dim_t height)
{
    ...
    egui_view_set_padding(EGUI_VIEW_OF(card), GRID_CARD_TEXT_PAD_X, GRID_CARD_TEXT_PAD_X, GRID_CARD_BODY_PAD_Y, GRID_CARD_BODY_PAD_Y);
    egui_view_set_background(EGUI_VIEW_OF(card), EGUI_BG_OF(&bg_card_neutral));

    init_text_label(eyebrow_label, width - GRID_CARD_TEXT_PAD_X * 2, 11, "", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT_MUTED,
    ...
    init_text_label(card_title_label, width - GRID_CARD_TEXT_PAD_X * 2, 11, "", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT,
                    EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER);
                           ...
                           egui_dim_t height, uint8_t visible)
{
    ...
    compact_card = height < 26 ? 1 : 0;
    egui_view_set_gone(EGUI_VIEW_OF(card), 0);
    egui_view_set_size(EGUI_VIEW_OF(card), width, height);
    egui_view_set_background(EGUI_VIEW_OF(card), grid_card_get_background(item->tone));
    egui_view_set_padding(EGUI_VIEW_OF(card), compact_card ? GRID_CARD_COMPACT_PAD_X : GRID_CARD_TEXT_PAD_X,
                          compact_card ? GRID_CARD_COMPACT_PAD_X : GRID_CARD_TEXT_PAD_X,
                          compact_card ? GRID_CARD_COMPACT_PAD_Y : GRID_CARD_BODY_PAD_Y,
                          compact_card ? GRID_CARD_COMPACT_PAD_Y : GRID_CARD_BODY_PAD_Y);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(card), compact_card ? EGUI_ALIGN_CENTER : EGUI_ALIGN_LEFT);

    egui_view_set_gone(EGUI_VIEW_OF(eyebrow_label), compact_card);
    egui_view_set_size(EGUI_VIEW_OF(eyebrow_label), width - GRID_CARD_TEXT_PAD_X * 2, compact_card ? 0 : 11);
    grid_set_fitted_label_text(eyebrow_label, eyebrow_buffer, eyebrow_buffer_size, compact_card ? "" : item->eyebrow, 4);
    egui_view_label_set_font_color(EGUI_VIEW_OF(eyebrow_label), grid_card_get_eyebrow_color(item->tone), EGUI_ALPHA_100);

    egui_view_set_size(EGUI_VIEW_OF(card_title_label), width - (compact_card ? GRID_CARD_COMPACT_PAD_X * 2 : GRID_CARD_TEXT_PAD_X * 2),
                       compact_card ? height - GRID_CARD_COMPACT_PAD_Y * 2 : 11);
    egui_view_label_set_align_type(EGUI_VIEW_OF(card_title_label), compact_card ? EGUI_ALIGN_CENTER : (EGUI_ALIGN_LEFT | EGUI_ALIGN_VCENTER));
...
void test_init_ui(void)
{
    ...
    init_text_label(&title_label, GRID_ROOT_WIDTH, 18, title_text, (const egui_font_t *)&egui_res_font_montserrat_12_4, HCW_COLOR_TEXT,
                    EGUI_ALIGN_CENTER);
    ...
    egui_view_set_margin(EGUI_VIEW_OF(&primary_grid), 0, 2, 0, 4);
    egui_view_group_add_child(EGUI_VIEW_OF(&primary_panel), EGUI_VIEW_OF(&primary_grid));

    for (index = 0; index < GRID_CARD_CAPACITY; index++)
    {
        init_card(&primary_cards[index], &primary_card_eyebrows[index], &primary_card_titles[index], 72, 30);
        grid_apply_centered_label_api(EGUI_VIEW_OF(&primary_card_eyebrows[index]), &primary_eyebrow_apis[index]);
        grid_apply_centered_label_api(EGUI_VIEW_OF(&primary_card_titles[index]), &primary_title_apis[index]);
    ...
    init_text_label(&stack_heading_label, 84, 12, "Stack", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT, EGUI_ALIGN_CENTER);
    ...
    for (index = 0; index < GRID_STACK_PREVIEW_CARDS; index++)
    {
        ...
        grid_apply_centered_label_api(EGUI_VIEW_OF(&stack_card_eyebrows[index]), &stack_eyebrow_apis[index]);
        grid_apply_centered_label_api(EGUI_VIEW_OF(&stack_card_titles[index]), &stack_title_apis[index]);
        egui_view_group_add_child(EGUI_VIEW_OF(&stack_preview_grid), EGUI_VIEW_OF(&stack_cards[index]));
    }

    init_text_label(&stack_note_label, 84, 10, "Static preview.", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT_MUTED,
                    EGUI_ALIGN_CENTER);
    ...
    init_text_label(&dense_heading_label, 84, 12, "Dense", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT, EGUI_ALIGN_CENTER);
    ...
    for (index = 0; index < GRID_DENSE_PREVIEW_CARDS; index++)
    {
        ...
        grid_apply_centered_label_api(EGUI_VIEW_OF(&dense_card_eyebrows[index]), &dense_eyebrow_apis[index]);
        grid_apply_centered_label_api(EGUI_VIEW_OF(&dense_card_titles[index]), &dense_title_apis[index]);
        egui_view_group_add_child(EGUI_VIEW_OF(&dense_preview_grid), EGUI_VIEW_OF(&dense_cards[index]));
    }

    init_text_label(&dense_note_label, 84, 10, "Three columns.", (const egui_font_t *)&egui_res_font_montserrat_8_4, HCW_COLOR_TEXT_MUTED,
                    EGUI_ALIGN_CENTER);
