...
static const egui_view_grid_splitter_snapshot_t compact_snapshots[] = {
        {"UI", "Compact split", "", "List", "6", "Left rail stays visible.", "Preview", "1:1", "Right pane keeps the detail block.", "", 46,
         EGUI_VIEW_GRID_SPLITTER_EMPHASIS_RIGHT},
        {"QA", "Compact audit", "", "Queue", "9", "Audit rows keep a wider left rail.", "Panel", "Lock", "Compact comparison stays static.", "", 56,
         EGUI_VIEW_GRID_SPLITTER_EMPHASIS_LEFT},
};
...
{
    egui_view_grid_splitter_set_current_snapshot(EGUI_VIEW_OF(&splitter_compact), index % COMPACT_SNAPSHOT_COUNT);
}
...
static int dismiss_primary_focus_on_preview_touch(egui_view_t *self, egui_motion_event_t *event)
{
    ...
    {
        egui_view_clear_focus(EGUI_VIEW_OF(&splitter_primary));
    }
#else
    EGUI_UNUSED(event);
#endif
...
void test_init_ui(void)
{
    ...
    egui_view_grid_splitter_set_snapshots(EGUI_VIEW_OF(&splitter_compact), compact_snapshots, COMPACT_SNAPSHOT_COUNT);
    ...
    splitter_compact_api.on_touch = dismiss_primary_focus_on_preview_touch;
#endif
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
...
#if EGUI_CONFIG_FUNCTION_SUPPORT_TOUCH
    splitter_read_only_api.on_touch = dismiss_primary_focus_on_preview_touch;
#endif
#if EGUI_CONFIG_FUNCTION_SUPPORT_FOCUS
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 10:
        if (first_call)
        {
            set_click_view_center(p_action, EGUI_VIEW_OF(&splitter_compact), 220);
        }
        return true;
    case 11:
        if (first_call)
