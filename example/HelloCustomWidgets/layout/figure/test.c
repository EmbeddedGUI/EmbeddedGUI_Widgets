...
#define FIGURE_ROOT_WIDTH        238
#define FIGURE_ROOT_HEIGHT       190
#define FIGURE_PRIMARY_WIDTH     184
#define FIGURE_PRIMARY_HEIGHT    80
#define FIGURE_CHILD_WIDTH       48
#define FIGURE_CHILD_HEIGHT      18
#define FIGURE_PREVIEW_ROW_WIDTH 196
#define FIGURE_PREVIEW_WIDTH     92
#define FIGURE_PREVIEW_HEIGHT    48
#define FIGURE_PREVIEW_CHILD_W   42
#define FIGURE_PREVIEW_CHILD_H   16
#define FIGURE_RECORD_WAIT       90
#define FIGURE_RECORD_FRAME_WAIT 170
#define FIGURE_RECORD_FINAL_WAIT 280
#define FIGURE_DEFAULT_SNAPSHOT  0

#define PRIMARY_SNAPSHOT_COUNT ((uint8_t)EGUI_ARRAY_SIZE(primary_snapshots))

typedef struct figure_snapshot figure_snapshot_t;
struct figure_snapshot
{
...
static void apply_figure_style(egui_view_t *view, uint8_t style)
{
    switch (style)
    {
    case 1:
        egui_view_figure_apply_accent_style(view);
        break;
    case 2:
        egui_view_figure_apply_compact_style(view);
        break;
    case 3:
        egui_view_figure_apply_read_only_style(view);
        break;
    default:
        egui_view_figure_apply_standard_style(view);
        break;
    }
}
...
{
    const figure_snapshot_t *snapshot = &primary_snapshots[index % PRIMARY_SNAPSHOT_COUNT];

    apply_figure_style(EGUI_VIEW_OF(&primary_control), snapshot->style);
    egui_view_figure_set_text(EGUI_VIEW_OF(&primary_control), snapshot->leading_text, snapshot->wrap_text, snapshot->trailing_text);
    ...
    egui_view_figure_layout_child(EGUI_VIEW_OF(&primary_control));
    if (ui_ready)
    {
        layout_page();
    }
...
{
    apply_primary_snapshot(FIGURE_DEFAULT_SNAPSHOT);
}
...
void test_init_ui(void)
{
    ...
    egui_view_set_size(EGUI_VIEW_OF(&root_layout), FIGURE_ROOT_WIDTH, FIGURE_ROOT_HEIGHT);
    ...
    egui_view_figure_init(EGUI_VIEW_OF(&primary_control));
    egui_view_set_size(EGUI_VIEW_OF(&primary_control), FIGURE_PRIMARY_WIDTH, FIGURE_PRIMARY_HEIGHT);
    ...
    egui_view_set_size(EGUI_VIEW_OF(&bottom_row), FIGURE_PREVIEW_ROW_WIDTH, FIGURE_PREVIEW_HEIGHT);
    egui_view_linearlayout_set_orientation(EGUI_VIEW_OF(&bottom_row), 1);
    egui_view_linearlayout_set_align_type(EGUI_VIEW_OF(&bottom_row), EGUI_ALIGN_VCENTER);
    egui_view_group_add_child(EGUI_VIEW_OF(&root_layout), EGUI_VIEW_OF(&bottom_row));

    egui_view_figure_init(EGUI_VIEW_OF(&compact_preview));
    ...
    egui_view_figure_init(EGUI_VIEW_OF(&read_only_preview));
    egui_view_set_size(EGUI_VIEW_OF(&read_only_preview), FIGURE_PREVIEW_WIDTH, FIGURE_PREVIEW_HEIGHT);
...
bool egui_port_get_recording_action(int action_index, egui_sim_action_t *p_action)
{
    ...
    case 0:
        ...
        EGUI_SIM_SET_WAIT(p_action, FIGURE_RECORD_FRAME_WAIT);
        return true;
    case 1:
        if (first_call)
        {
        ...
        EGUI_SIM_SET_WAIT(p_action, FIGURE_RECORD_WAIT);
        return true;
    case 2:
        if (first_call)
        {
    ...
    case 7:
        ...
        EGUI_SIM_SET_WAIT(p_action, FIGURE_RECORD_FINAL_WAIT);
        return true;
    default:
        break;
    }
