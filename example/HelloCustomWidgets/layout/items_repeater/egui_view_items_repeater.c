...
{
    egui_dim_t base_h = local->compact_mode ? EGUI_VIEW_ITEMS_REPEATER_COMPACT_ITEM_H : EGUI_VIEW_ITEMS_REPEATER_STANDARD_ITEM_H;
...
static egui_dim_t items_repeater_pill_width(const char *text, uint8_t compact_mode, egui_dim_t min_width, egui_dim_t max_width)
{
    egui_dim_t width = min_width;

    if (items_repeater_has_text(text))
    {
        width += items_repeater_text_len(text) * (compact_mode ? 4 : 5);
    }
    if (width > max_width)
    {
        width = max_width;
...
static egui_dim_t items_repeater_item_width(egui_view_items_repeater_t *local, const egui_view_items_repeater_item_t *item, egui_dim_t max_width)
{
    ...
    {
        width += 4 + items_repeater_text_len(item->badge) * 2;
    }
    if (items_repeater_has_text(item->title))
    {
        width += 6 + items_repeater_text_len(item->title) * 3;
    }
    if (items_repeater_has_text(item->meta))
    {
        width += 4 + items_repeater_text_len(item->meta) * 2;
    }
                                   ...
                                   egui_view_items_repeater_metrics_t *metrics)
{
    ...
    egui_dim_t badge_h = local->compact_mode ? EGUI_VIEW_ITEMS_REPEATER_COMPACT_BADGE_H : EGUI_VIEW_ITEMS_REPEATER_STANDARD_BADGE_H;
    ...
    egui_dim_t title_h = local->compact_mode ? EGUI_VIEW_ITEMS_REPEATER_COMPACT_TITLE_H : EGUI_VIEW_ITEMS_REPEATER_STANDARD_TITLE_H;
    egui_dim_t summary_h = local->compact_mode ? EGUI_VIEW_ITEMS_REPEATER_COMPACT_SUMMARY_H : EGUI_VIEW_ITEMS_REPEATER_STANDARD_SUMMARY_H;
    ...
    egui_dim_t footer_h = local->compact_mode ? EGUI_VIEW_ITEMS_REPEATER_COMPACT_FOOTER_H : EGUI_VIEW_ITEMS_REPEATER_STANDARD_FOOTER_H;
    ...
    if (items_repeater_has_text(snapshot->header))
    {
        ...
        metrics->badge_region.size.width = items_repeater_pill_width(snapshot->header, local->compact_mode, local->compact_mode ? 18 : 24, inner_w);
    ...
    if (footer_h > 0 && items_repeater_has_text(snapshot->footer))
    {
        ...
        metrics->footer_region.size.width = items_repeater_pill_width(snapshot->footer, local->compact_mode, local->compact_mode ? 18 : 22, inner_w);
                                 ...
                                 uint8_t item_index)
{
    ...
    if (items_repeater_has_text(item->badge))
    {
        badge_w = items_repeater_pill_width(item->badge, local->compact_mode, local->compact_mode ? 12 : 16, region->size.width / 2);
    ...
    if (items_repeater_has_text(item->meta))
    {
        meta_w = 8 + items_repeater_text_len(item->meta) * (local->compact_mode ? 3 : 4);
