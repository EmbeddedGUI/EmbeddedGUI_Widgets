...
- 示例页结构收敛为标题、标准态主卡和 compact / read-only 双预览，不再保留页面级 guide / 状态栏 / section label。
...
- 底部对照容器尺寸：`216 x 64`
- 顶部结构：标题、标准态主卡、双预览
- 主卡要求：边框、弱阴影、eyebrow pill、counter pill、title、footer 均需完整可见
- 按钮要求：previous / next 的 chevron 需要真实居中，左右内边距平衡
- 风格要求：回到浅底、轻边框、低饱和 accent 的 Fluent / WPF UI 单卡轮播语义，不保留紫色 hero 卡或页面级说明条。

## 5. 控件清单

| 变量名 | 类型 | 尺寸 | 初始状态 | 用途 |
| --- | --- | --- | --- | --- |
| `flip_view_primary` | `egui_view_flip_view_t` | `196 x 122` | `Stories track / index=1` | 标准主视图 |
...
| --- | --- | --- | --- |
| `Stories` | `Night city` | 默认焦点页 | 标准 hero card 浏览 |
| `Stories` | `Quiet review` | 尾页边界 | next 按钮禁用 |
| `Planner` | `Review board` | 中间翻页态 | 单卡轮播，不显示透视侧卡 |
| `Archive` | `Paper trail` | 只读对照参考 | muted shell + disabled arrows |
| `Compact` | `Pocket` | 小尺寸收口 | helper 隐藏、布局压缩 |
...
- 程序化切换主数据轨到 `Planner`，验证标题与主卡内容同步变化。
- 程序化切换 compact 数据轨，验证小尺寸对照态同步更新。
...
- 删除了页面级 guide / 状态条等非控件本体说明 chrome。
