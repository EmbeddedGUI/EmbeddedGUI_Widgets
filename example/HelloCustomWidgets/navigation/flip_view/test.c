...
static const egui_view_flip_view_item_t highlights_items[] = {
        {"Overview", "Q1 summary", "One active card keeps the current update in focus", "Primary surface stays centered", HCW_COLOR_PANEL,
         HCW_COLOR_PRIMARY},
        {"Overview", "Shipping board", "Previous and next stay secondary to content", "Navigation remains low-noise", HCW_COLOR_PANEL,
         HCW_COLOR_TEXT_MUTED},
        {"Overview", "Release note", "Compact preview reuses the same card shell", "Preview remains restrained", HCW_COLOR_PANEL,
         HCW_COLOR_SUCCESS},
        {"Overview", "Archive handoff", "Read-only freezes previous and next", "Boundary state stays explicit", HCW_COLOR_PANEL,
         HCW_COLOR_WARNING},
};

static const egui_view_flip_view_item_t operations_items[] = {
        {"Ops", "Planning board", "Sequential browsing replaces stacked panels", "Single surface, ordered review", HCW_COLOR_PANEL,
         HCW_COLOR_PRIMARY},
        {"Ops", "Status deck", "Content changes without changing the shell", "Same frame across snapshots", HCW_COLOR_PANEL,
         HCW_COLOR_TEXT_MUTED},
        {"Ops", "Review queue", "Compact mode trims helper rows first", "Smaller shell, same semantics", HCW_COLOR_PANEL,
         HCW_COLOR_PRIMARY},
};

static const egui_view_flip_view_item_t records_items[] = {
        {"Record", "Case note", "Metadata stays short and readable", "No image asset required", HCW_COLOR_PANEL, HCW_COLOR_WARNING},
        {"Record", "Approval", "Footer reflects the active item only", "State stays lightweight", HCW_COLOR_PANEL, HCW_COLOR_SUCCESS},
        {"Record", "Retention", "Track swaps do not change the widget shell", "Pager stays secondary", HCW_COLOR_PANEL, HCW_COLOR_TEXT_MUTED},
        {"Record", "Readout", "Read-only preview keeps arrows disabled", "State boundary remains visible", HCW_COLOR_PANEL, HCW_COLOR_WARNING_DARK},
};

static const egui_view_flip_view_item_t compact_overview_items[] = {
        {"Compact", "Snapshot", "Compact keeps only essential text", "", HCW_COLOR_PANEL, HCW_COLOR_PRIMARY},
        {"Compact", "Queue", "Navigation remains secondary", "", HCW_COLOR_PANEL, HCW_COLOR_PRIMARY},
        {"Compact", "Review", "Footer still anchors context", "", HCW_COLOR_PANEL, HCW_COLOR_TEXT_MUTED},
};

static const egui_view_flip_view_item_t read_only_items[] = {
        {"Read", "Summary", "Navigation is intentionally disabled", "", HCW_COLOR_PAGE_BG, HCW_COLOR_TEXT_SOFT},
        {"Read", "Snapshot", "Overlay arrows stay dimmed", "", HCW_COLOR_PAGE_BG, HCW_COLOR_TEXT_SOFT},
        {"Read", "Record", "Card layout remains unchanged", "", HCW_COLOR_PAGE_BG, HCW_COLOR_TEXT_SOFT},
};
...
void test_init_ui(void)
{
    ...
    egui_view_flip_view_set_palette(EGUI_VIEW_OF(&flip_view_primary), HCW_COLOR_SURFACE, HCW_COLOR_BORDER, HCW_COLOR_TEXT,
                                    HCW_COLOR_TEXT_MUTED, HCW_COLOR_TEXT_SOFT);
    ...
    egui_view_flip_view_set_compact_mode(EGUI_VIEW_OF(&flip_view_compact), 1);
    egui_view_flip_view_set_palette(EGUI_VIEW_OF(&flip_view_compact), HCW_COLOR_SURFACE, HCW_COLOR_BORDER, HCW_COLOR_TEXT,
                                    HCW_COLOR_TEXT_MUTED, HCW_COLOR_TEXT_SOFT);
    ...
    egui_view_flip_view_set_compact_mode(EGUI_VIEW_OF(&flip_view_read_only), 1);
    egui_view_flip_view_set_read_only_mode(EGUI_VIEW_OF(&flip_view_read_only), 1);
    egui_view_flip_view_set_palette(EGUI_VIEW_OF(&flip_view_read_only), HCW_COLOR_PANEL, HCW_COLOR_BORDER, HCW_COLOR_TEXT_MUTED,
