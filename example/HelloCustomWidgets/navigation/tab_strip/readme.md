...
- 本次删除效果：页面级 `guide`、旧 preview 标签和双列包裹、让 preview 继续承担切换职责的桥接逻辑、过重 active tab 与 underline 强调
- EGUI 适配说明：继续复用仓库内 `tab_strip` 基础实现，本轮重点收口 `reference` 页面结构、same-target release、static preview 语义和 pressed 清理，不修改 `sdk/EmbeddedGUI`
...
- 样式约束：
  ...
  - 当前项保留轻量 fill 和 underline，但降低 active fill、文字染色和双层底线强度。
  - `read only` 使用弱化灰蓝 palette，只做静态 reference 对照。
...
- `set_tabs()`、`set_current_index()`、`set_font()`、`set_palette()`、`set_compact_mode()`、`set_read_only_mode()` 都会先清理残留 `pressed_index / is_pressed`。
- 主控件切到 `compact_mode` 时仍保留标签切换能力，但必须先清掉残留 pressed。
- `read_only_mode`、`!enable` 收到新的 touch / key 输入时，会先清掉残留 pressed，再拒绝提交。
- 底部 preview 统一通过 `egui_view_tab_strip_override_static_preview_api()` 收口：
  - 吞掉 touch / key 输入。
...
- 当前键盘交互只覆盖 `Left / Right / Up / Down / Home / End` 的主线导航。
