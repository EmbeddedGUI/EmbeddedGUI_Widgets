...
`title_bar` 用来表达页面或工作区顶部的导航与状态入口。它不是完整的系统窗口栏复刻，而是一个贴近 Fluent / WPF UI `TitleBar` 语义的轻量 reference 容器，负责承载返回、pane toggle、标题、副标题和右侧动作入口。
...
| `primary_snapshots` | `egui_view_title_bar_snapshot_t[4]` | - | `Atlas / Files / Release / Read back` | 主控件录制轨道 |
| `compact_snapshots` | `egui_view_title_bar_snapshot_t[2]` | - | `Atlas / Files` | compact 对照 |
...
| `compact` | `Atlas` | 默认 compact 对照 |
| `compact` | `Files` | 程序化切换后的 compact 对照 |
| `read only` | `Preview` | 固定只读对照 |
...
- 底部 preview 通过 `egui_view_title_bar_override_static_preview_api()` 收口：
  - 吞掉 `touch / key`
  - 只清残留 pressed，不改变 snapshot 和 current part
  - 不触发 action listener
...
7. 点击主控件主动作
8. 输出第四张截图
9. 切换 `compact` preview 到第二组状态，并给主控件加回焦点
10. 输出带焦点的对照截图
11. 点击 `compact` preview，验证 static preview 只做清焦收尾
12. 输出最终稳定帧
...
- `compact` 必须收掉多余信息密度，而不是另起一个展示壳层。
- same-target release 必须通过，不能在 `ACTION_MOVE` 改写 `pressed_part`。
- `read only` 与 static preview 需要吞掉输入并清掉残留 pressed。
...
- static preview 只负责清 pressed / 清焦点，不改变内部 snapshot。
